...
#define FIDX 0x5A   /* Unify WoBLE APCF Filtering Index for MT7668*/
#define FIDX_7663 0x0A /* Unify WoBLE APCF Filtering Index for MT76x3 */
...
//---------------------------------------------------------------------------
static int btmtk_usb_io_read32_76xx(struct LD_btmtk_usb_data *data, u32 reg, u32 *val)
{
    int ret = -1;
    __le16 reg_high;
    __le16 reg_low;
...
static int btmtk_usb_send_hci_set_ce_cmd(struct LD_btmtk_usb_data *data)
{
    ...
    if (ret < 0) {
        ...
        usb_debug("skip it, got response length(%d)\n", ret);
        return -1;
    }
...
static void btmtk_usb_cap_init(struct LD_btmtk_usb_data *data)
{
    ...
    if (data->chip_id == 0)
        btmtk_usb_io_read32_76xx(data, 0x80000008, &data->chip_id);
    ...
    if (is_mt7630(data) || is_mt7650(data)) {
        ...
        btmtk_usb_io_read32_76xx(data, 0x80000004, &fw_ver);
        if ((fw_ver & 0xFF) != 0xFF) {
            data->rom_patch_bin_file_name = os_kzalloc(MAX_BIN_FILE_NAME_LEN, MTK_GFP_ATOMIC);
            if (!data->rom_patch_bin_file_name) {
                usb_debug("Can't allocate memory\n");
...
load_patch_protect:
    ...
    if (!data->rom_patch) {
        usb_debug("please assign a rom patch(/etc/firmware/%s)or(/lib/firmware/%s)\n",
...
static int btmtk_usb_set_apcf(struct LD_btmtk_usb_data *data, BOOL bin_file)
{
    ...
    // 7663 woble APCF index is 0x0A
    if(is_mt7663(data)) {
       manufacture_data[5] = FIDX_7663;
       filter_cmd[5] = FIDX_7663;
    }
    ...
    if (bin_file) {
        if (data->wake_dev_len) {
            ...
            // 7663 woble APCF index is 0x0A
             if(is_mt7663(data)) {
               broadcast_addr[5] = FIDX_7663;
               adv_pattern[5] = FIDX_7663;
             }
...
//only for 7663, this cmd is used to adjust FW memory region after load FW bin
static int btmtk_usb_send_wmt_cfg(struct LD_btmtk_usb_data *data)
{
    /*wmt cmd, only for 7663*/
    u8 cmd[] = { 0x6F, 0xFC, 0x06, 0x01, 0x06, 0x02, 0x00, 0x03, 0x01 };
    int ret = -1;

    usb_debug("send wmt cfg cmd!\n");
    ret = btmtk_usb_send_wmt_cmd(data, cmd, sizeof(cmd), NULL, 0, 100, 10);
    if (ret < 0) {
        ...
        void *value, u8 pos, u8 len)
{
    ...
    if (radiooff == NULL || value == NULL || len == 0) {
        usb_debug("%s: Incorrect parameters - %s %s %s\n", __func__,
                radiooff ? "" : "radiooff",
                value ? "" : "value", len ? "" : "len");
        return -EINVAL;
    }
        ...
        if (i < radiooff->len - 1 && radiooff->value[i + 1] == attr) {
            if (attr == WOBX_TYPE_KEYCODE_MAPPING) {
                if (len != attr_len - 1) {
                    ...
                    usb_debug("%s: Re-allocate size(%d/%d) for mod\n", __func__,
    ...
    if (attr == WOBX_TYPE_IR) {
        usb_debug("%s: Please add WOBX_TYPE_IR in woble_setting.bin first\n", __func__);
        return -ENOENT;
...
#define CHAR2DEC_SIZE   2
    ...
    if (radiooff == NULL || setting == NULL) {
        usb_debug("%s: Incorrect parameters - %s %s\n", __func__,
    ...
    usb_debug("%s: mark: 0x%04X\n", __func__, mark);

    for (i = 0; ((mark >> i) & 0x0001) && i < APCF_SETTING_COUNT; i++) {
        usb_debug("%s: The following is radiooff[%d]\n", __func__, i);
        /* IR protocol parsing */
        ...
        if (head) {
            ...
            if (head) {
                ...
                /* # IR Protocols
                   ...
                   #     RC5=0x01,
                   #     RC6=0x02,
                   So we can't consider strtol return 0 is fail case, and mboot doesn't support kstrtoul().
                 */
                ir_value = strtol(ir, ptr, 0);
                usb_debug("%s: ir protocol is 0x%02X\n", __func__, (int)ir_value);
            ...
            if (head) {
                ...
                if (group_value) {
                    ...
                    if (group_value > MAX_IRKMG) {
                        usb_debug("%s: Key mapping can't over than %d groups(%d), ignore surplus\n",
                    ...
                    for (j = 1; j <= group_value; j++) {
                        ...
                        if (head) {
                            ...
                            usb_debug("%s: Can't find keymap[%d]\n", __func__, j);
                            break;
                        }
                    }
                ...
                usb_debug("%s: Incorrect format\n", __func__);
        } else
            usb_debug("%s: No keymap[%d]\n", __func__, i);
    }
    return 0;
}
...
int btmtk_usb_load_woble_setting(struct LD_btmtk_usb_data *data)
{
    ...
    LD_load_code_from_bin(&data->woble_setting, data->woble_setting_file_name, NULL,
...
int btmtk_usb_set_unify_woble(struct LD_btmtk_usb_data *data)
{
    ...
    if (data->radioff_cmd[0].len && data->radioff_cmd[0].value != NULL) {
        ...
        usb_debug("Using default radio off cmd\n");
        ret = btmtk_usb_send_hci_cmd(data, cmd, sizeof(cmd), event, sizeof(event));
    }
    if (ret < 0)
        usb_debug("Failed(%d)\n", ret);

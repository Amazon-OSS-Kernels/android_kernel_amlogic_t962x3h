...
unsigned int get_kernel_log_level(char * cmdline)
{
	char flags_buf[IDME_FLAGS_LEN+1] = {0};
	unsigned kernel_loglevel = 0;
	unsigned long long fos_flags = 0;

	if (BUILD_VARIANT_USERDEBUG == getbuildvariant(cmdline)) {
		kernel_loglevel = 7;
		printf("user debug !\n");
	}else {
		...
		if ((fos_flags & FOS_FLAGS_CONSOLE_ON) == FOS_FLAGS_CONSOLE_ON) {
			printf("enable uart !\n");
			kernel_loglevel = 7;
		}else {
			printf("disable uart !\n");
...
bool amzn_dm_verity_is_off(int unlock_status)
{
	...
	if (lock_state) {
		/* dm-verity should be off in diag mode due to dfs can not be built in verity way */
...
#define MAX_UNLOCK_STR_SIZE 150
	...
	const char * console_config = "console=ttyS0,460800";

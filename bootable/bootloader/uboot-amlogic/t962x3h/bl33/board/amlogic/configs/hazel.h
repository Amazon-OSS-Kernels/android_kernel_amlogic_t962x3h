...
/* support U-disk triggered standby for primrose*/
#define UBOOT_TARGET_PRODUCT_NAME_PRIMROSE
...
#define HVT_BOARD_ID   "00980001000A0019"
#define HVT2_BOARD_ID  "00980001000A0020"
#define EVT_BOARD_ID   "00980002000A0019"
#define DVT_BOARD_ID   "00980003000A0019"
#define PVT_BOARD_ID   "00980014000A0019"
...
#define CONFIG_IR_REMOTE_POWER_UP_KEY_VAL14     0xf20dbf00
#define CONFIG_IR_REMOTE_POWER_UP_KEY_VAL15	0x5ba47d02 /* ABS presetting1 key */
#define CONFIG_IR_REMOTE_POWER_UP_KEY_VAL16	0x5aa57d02 /* ABS presetting2 key */
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"gamma_setting=none\0" \
        ...
        "cec_init="\
            ...
            "\0"\
        "initargs="\
            "init=/init console=ttyS0,460800 no_console_suspend earlycon=aml-uart,0xff803000 printk.devkmsg=on ramoops.pstore_en=1 ramoops.record_size=0x8000 ramoops.console_size=0x4000 "\
            "\0"\
        "upgrade_check="\
            ...
            "\0"\
        "storeargs="\
            "setenv bootargs ${initargs} ${fs_type} logo=${display_layer},loaded,${fb_addr} powermode=${powermode} vout=${outputmode},enable panel_type=${panel_type} lcd_ctrl=${lcd_ctrl} hdmimode=${hdmimode} cvbsmode=${cvbsmode} osd_reverse=${osd_reverse} video_reverse=${video_reverse} androidboot.selinux=${EnableSelinux} androidboot.firstboot=${firstboot} androidboot.rpmb_state=${rpmb_state} jtag=${jtag} androidboot.support_memc=${support_memc}; "\
            "setenv bootargs ${bootargs} gamma_setting=${gamma_setting}; "\
            ...
            "setenv bootargs ${bootargs} usb_power=${usb_power};" \
  ...
  /*          "get_valid_slot;"\   */\
        ...
        "irremote_update="\
            "if irkey 0 0x0; then "\
                "echo read irkey ok!; " \
                "if usb start 0; then "\
                    "if fatload usb 0 ${loadaddr} usb_burn_package.img 200; then "\
                        "if itest ${irkey_value} == 0x0; then " \
                            "run init_display;"\
                            "run storeargs;"\
                            "run upgrade_usb;"\
                            "run recovery_from_flash;"\
...
//primrose don't support  local dimming
...
#if defined(CONFIG_DEVICE_PRODUCT_PRIMROSE)
...
#define CONFIG_DEVICE_PRODUCT   "primrosebo"
#endif

...
{
	/*set gpioZ_1 ~ 3 low*/
	writel(readl(P_PREG_PAD_GPIO1_O) & (~(0x7 << 1)), P_PREG_PAD_GPIO1_O);
	writel(readl(P_PREG_PAD_GPIO1_EN_N) & (~(0x7 << 1)), P_PREG_PAD_GPIO1_EN_N);
	writel(readl(P_PERIPHS_PIN_MUX_2) & (~(0xfff << 4)), P_PERIPHS_PIN_MUX_2);
#if 0
...
#else
	/*set gpioAO_8 low to power off VDD5V and hdmi_3.3v*/
...
static void power_on_at_24M(unsigned int suspend_from)
{
	...
	writel(readl(AO_RTI_PINMUX_REG0) & (~(0xf << 12)), AO_RTI_PINMUX_REG0);
#else
	/*set gpioAO_8 high to power on VDD5V and hdmi_3.3v*/
	writel(readl(AO_GPIO_O) | (1 << 8), AO_GPIO_O);
	writel(readl(AO_GPIO_O_EN_N) & (~(1 << 8)), AO_GPIO_O_EN_N);
	writel(readl(AO_RTI_PINMUX_REG1) & (~(0xf<<0)), AO_RTI_PINMUX_REG1);
#endif
...
void get_wakeup_source(void *response, unsigned int suspend_from)
{
	...
	val = (POWER_KEY_WAKEUP_SRC | AUTO_WAKEUP_SRC | REMOTE_WAKEUP_SRC |
	       ETH_PMT_WAKEUP_SRC | BT_WAKEUP_SRC | CECB_WAKEUP_SRC);
...
static unsigned int detect_key(unsigned int suspend_from)
{
	...
	do {
		...
		if (irq[IRQ_AO_IR_DEC] == IRQ_AO_IR_DEC_NUM) {
			...
			switch (remote_detect_key()) {
				case CONFIG_IR_REMOTE_POWER_UP_KEY_VAL16:
...
#ifdef CONFIG_WIFI_WAKEUP
		if (irq[IRQ_GPIO0] == IRQ_GPIO_NOW_0_NUM) {
	...
	writel(readl(AO_RTI_PINMUX_REG0)&(~(1 << 22)), AO_RTI_PINMUX_REG0);/* clear pwm aoa function */
	return exit_reason;
}

		...
		if (!value) {
			...
			if (value) {
				...
				if(tmp==NULL)
					...
					snprintf(tmp+ln, ln+strlen(value)+2-ln,"=%s", value);
					name = tmp;
				}
			} else {
...
static int builtin_read(struct child_prog *child)
{
	...
	if (child->argv[1]) {
		...
		if(var) {
			snprintf(var, strlen(child->argv[1])+strlen(string)+2, "%s=%s", child->argv[1], string);
...
static inline void setup_prompt_string(int promptmode, char **prompt_str)
{
	...
	if (promptmode == 1) {
		...
		snprintf(PS1, strlen(cwd)+4, "%s %s", cwd, ( geteuid() != 0 ) ?  "$ ":"# ");
		*prompt_str = PS1;
	} else {
		*prompt_str = PS2;
	}
...
static void insert_bg_job(struct pipe *pi)
{
	...
	/*if (pi->progs[0] && pi->progs[0].argv && pi->progs[0].argv[0]) */
	{
		...
		while(foo && *foo) {
			bar += snprintf(bar, BUFSIZ-strlen(thejob->text), "%s ", *foo++);
		}
	}
...
static char *lookup_param(char *src)
{
	...
	if (!p || strlen(p) == 0) {
		...
		if (assign) {
			...
			if (var) {
				snprintf(var, strlen(src)+strlen(default_val)+2, "%s=%s", src, default_val);
				set_local_var(var, 0);
			}
			free(var);
		}
...
static char *get_dollar_var(char ch)
{
	...
	switch (ch) {
		case '?':
			snprintf(buf, sizeof(buf), "%u", (unsigned int)last_return_code);
			break;
		default:
			return NULL;
	}

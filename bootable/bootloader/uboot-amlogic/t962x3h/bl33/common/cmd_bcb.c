...
#define CMD_FOURCEUPGRADE   "force_upgrade"
...
#define MISC_ACTION_ERASE 1 /*erase misc partition*/
#define MISC_ACTION_NONE 0  /*None*/
...
static int check_misc_erase(char * miscbuf, int size) 
{
    ...
    free(msg);
    return ((*read_times > MAX_READ_TIMES) ? MISC_ACTION_ERASE : MISC_ACTION_NONE);
}
    ...
    char * const argv[])
{
    ...
    if (!memcmp(command_mark, CMD_WIPE_DATA, strlen(command_mark))) {
        ...
        store_write_ops((unsigned char *)partition, (unsigned char *)miscbuf, 0, sizeof(miscbuf));
    } else if (!memcmp(command_mark, CMD_FOURCEUPGRADE, strlen(command_mark))) {
        printf("write cmd to enter recovery for force upgrade \n");
        if(amzn_target_is_lockdown()){
            printf("target is lockdonw , please unlock!\n");
            goto ERR;
        }
        memcpy(miscbuf, CMD_RUN_RECOVERY, sizeof(CMD_RUN_RECOVERY));
        memcpy(miscbuf+sizeof(command)+sizeof(status), "recovery\n--update_package=/udisk/update.bin", sizeof("recovery\n--update_package=/udisk/update.bin"));
        store_write_ops((unsigned char *)partition, (unsigned char *)miscbuf, 0, sizeof(miscbuf));
    }

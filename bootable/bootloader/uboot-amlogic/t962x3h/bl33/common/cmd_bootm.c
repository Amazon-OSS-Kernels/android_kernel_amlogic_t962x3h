...
static void defendkey_process(void)
{
	...
	if (!strstr(bootargs,"defendkey"))
	{
		...
		upgrade_step_s = get_logoparam_value_ex("logoparam.var.upgrade_step", true, false);
#else
...
/*Check rpmb state and add arg "androidboot.rpmb_state=1" in bootargs*/
#if !defined(CONFIG_AML_RPMB_DISABLE)
	run_command("rpmb_state",0);
#endif
    printf("bl:add uboot bl status\n");
    append_bootargs("bl_status", get_lcd_bl_status() ? "1":"0");

#ifdef CONFIG_DEVICE_PRODUCT_HAZEL
    bool lockdown_flag = false;
    lockdown_flag = amzn_target_is_lockdown();
    printf("lockdown_flag = %d\n", lockdown_flag);
    printf("Add fiq_check_en field to bootargs\n");
    append_bootargs("fiq_check_en", ((lockdown_flag == false) ? "1":"0"));
#endif

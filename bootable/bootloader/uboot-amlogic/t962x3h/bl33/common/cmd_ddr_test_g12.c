...
void dwc_window_reg_after_training_update(char over_ride_index,uint32_t over_ride_sub_index,uint32_t over_ride_value)
{
	...
	if (over_ride_index == DMC_TEST_WINDOW_INDEX_RXENDLY) {
		reg_add = ((0 << 20) | (1 << 16) | (((over_ride_sub_index % 8) >> 1) << 12) | (0x80 + (over_ride_sub_index / 8) + ((over_ride_sub_index % 2) << 8)));
		delay_old_value = dwc_ddrphy_apb_rd(reg_add);
		dwc_ddrphy_apb_wr(reg_add, delay_reg_value);
	}
...
if (argc>3)
	...
	unsigned int  n_dram_data_odt_ohm = 0;
...
unsigned int max_counter_total=(max_counter_loop_w1*max_counter_loop_w2+max_counter_loop_r1*max_counter_loop_r2+2)*max_counter_loop_wr1;
	...
	*/
	...
	if ((counter_loop%(max_counter_loop_w1*max_counter_loop_w2+max_counter_loop_r1*max_counter_loop_r2+2))<(max_counter_loop_w1*max_counter_loop_w2))
	{
		...
		n_dram_data_odt_ohm = (((counter_loop % (max_counter_loop_w1 * max_counter_loop_w2 + max_counter_loop_r1 * max_counter_loop_r2 + 2))) / max_counter_loop_w1);
		if (n_dram_data_odt_ohm > max_counter_loop_w2 -1) {
		    n_dram_data_odt_ohm = max_counter_loop_w2 - 1;
		}
		dram_data_odt_ohm = p_dram_data_odt_ohm[n_dram_data_odt_ohm];
	}
...
int do_ddr_auto_fastboot_check(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	char ddr_fastboot[PROD_VAR_SIZE+1] = {0,};

    idme_get_oem_data_field("ddr_fastboot=", ddr_fastboot, PROD_VAR_SIZE);
	printf("\nddr_fastboot==%s \n",ddr_fastboot);
	if (strncmp(ddr_fastboot,"1",1) != 0)
	{
	    printf("\nddr fastboot disable \n");
		return 1;
	} else 
	{
	    printf("\nddr fastboot enable !! \n");

...
#if defined(CONFIG_UBOOT_LOGGER)
static struct uboot_log  ulog_buffer;
unsigned long uboot_log_base(void)
{
	return &ulog_buffer;
}

int init_uboot_log_struct()
{
	struct uboot_log * log_buffer;
	log_buffer = (struct uboot_log *)uboot_log_base();
	log_buffer->header.cookie = UBOOT_LOG_COOKIE;
	log_buffer->header.max_size = UBOOT_LOG_BUF_SIZE;
	log_buffer->header.size_written = 0;
	log_buffer->header.idx = 0;
	memset(log_buffer->data,0,sizeof(log_buffer->data));
        printf("------init_uboot_log_struct-----\n");

}

unsigned *get_uboot_log_buf_addr()
{
	return (unsigned *)uboot_log_base();
}

unsigned get_uboot_log_buf_size()
{
	return ((struct uboot_log *)uboot_log_base())->header.max_size;
}

#define UBOOT_LOG_DATA_END     (UBOOT_LOG_DATA_LENGTH - 8)

static void ubootlog_to_buf(const char *s)
{
	int len, end;
	int i = 0;
	int log_size = UBOOT_LOG_DATA_LENGTH ;
	struct uboot_log * log =(struct uboot_log *)uboot_log_base();
	if (!get_uboot_log_buf_addr())
		return;
	if (log->header.idx < UBOOT_LOG_DATA_END) {
		len = strlen(s);
		end = log->header.idx + len;
		if (end < UBOOT_LOG_DATA_END) {
			while (log->header.idx < end)
				log->data[log->header.idx++] = s[i++];
		} else {
			end = UBOOT_LOG_DATA_END;
			while (log->header.idx < end)
				log->data[log->header.idx++] = s[i++];
			while (log->header.idx < end + 6)
				log->data[log->header.idx++] = '.';
			log->data[log->header.idx++] = '\n';
		}
		log->header.size_written =log->header.idx;
	}else{
		log->header.idx = 0;
	}

...
const char *model_config_file[256] = {0};
const char *model_config_file_vb1[256] = {0};
				...
				if ((((p_attr->cmd_data[j] >> 4) & 0xf) == 0xb) ||
					...
					if (ret == 0) {
						...
						if (data_buf[0]) {
							...
							reg_start_tmp = tmp_buf[i + 2];
							memcpy(&p_attr->cmd_data[j + 2],
								&data_buf[1], data_size);
							if (flag == 3) {
								p_attr->cmd_data[j + 2] = reg_start_tmp;
							}
...
static int handle_panel_misc(struct panel_misc_s *p_misc)
{
	...
	ini_value = IniGetString("panel_misc", "gamma_setting", "null");
	printf("%s, gamma_setting is (%s)\n", __FUNCTION__, ini_value);
	if (strcmp(ini_value, "yes") == 0) {
	run_command("setenv gamma_setting true", 0);
	}
			   ...
			   unsigned char *tcon_spi_buf)
{
	...
	unsigned int lcd_size = 0, lcd_buf_len = 0;
	struct lcd_header_s *header;

	lcd_buf_len = sizeof(struct lcd_attr_s) + sizeof(struct lcd_v2_attr_s);
	...
	if (lcd_size <= lcd_buf_len) {
		memcpy((void *)lcd_buf, (void *)&lcd_attr, lcd_attr.head.block_cur_size);
	} else {
		ALOGE("%s, lcd_size(0x%x) > lcd_buf_len(0x%x)!\n", __func__, lcd_size, lcd_buf_len);
		return -1;
	}
	/* handle lcd_v2 attr*/
...
#define VCOM_DATA_SIZE_MAX (LCD_EXTERN_INIT_ON_MAX - 18)
extern int get_vcom_data(char *data, int len, int index); // board/amlogic/hazel/hazel.c
static int handle_tcon_ext_pmu_data(int index, int flag, unsigned char *buf,
				    unsigned int offset, unsigned int data_len)
{
	char str[2][30];
	...
	char *ext_data = NULL, *vcom = NULL;
	int vcom_start = 0, vcom_len = 0, ret = 0;
	//model_debug_flag = 255;
	...
	/* for amazon vcom patch */
	if (flag == 3) {
		ext_data = (char *)malloc(LCD_EXTERN_INIT_ON_MAX);
		if (!ext_data)
			goto err_exit;
		memset(ext_data, 0, LCD_EXTERN_INIT_ON_MAX);

		vcom = (char *)malloc(VCOM_DATA_SIZE_MAX);
		if (!vcom)
			goto err_exit;
		memset(vcom, 0, VCOM_DATA_SIZE_MAX);

		if (get_vcom_data(ext_data, LCD_EXTERN_INIT_ON_MAX, index)) {
			ALOGD("%s: can't get vcom data\n", __func__);
			goto err_exit;
		} else {
			ALOGD("%s: get vcom data %s\n", __func__, ext_data);
		}

		sprintf(str[0], "TCON_B%d_VCOM_DATA", index);
		if (strncmp(ext_data, str[0], strlen(str[0])) == 0) {
			vcom_start = strlen(str[0]) + 1; /* include '='*/
			vcom_len = strlen(ext_data) - vcom_start;
			if ((vcom_len & 0x1) || (vcom_len <= 0)) {
				ALOGE("%s: err length of vcom data %s, len:%d\n",
				__func__, ext_data, vcom_len);
				goto err_exit;
			}
			ALOGD("vcom_start:%d, vcom_len:%d\n", vcom_start, vcom_len);
			for (i = 0; i < vcom_len; i++) {
				if ((ext_data[vcom_start + i] >= '0') &&
					(ext_data[vcom_start + i] <= '9'))
					vcom[i] = ext_data[vcom_start + i] - '0';
				else if ((ext_data[vcom_start + i] >= 'a') &&
					(ext_data[vcom_start + i] <= 'f'))
					vcom[i] = ext_data[vcom_start + i] - 'a' + 0xa;
				else if ((ext_data[vcom_start + i] >= 'A') &&
					(ext_data[vcom_start + i] <= 'F'))
					vcom[i] = ext_data[vcom_start + i] - 'A' + 0xA;
				else
			...
			buf[0] = vcom_len / 2 + 1; //inlcude start reg
			for (i = 0; i < vcom_len / 2; i++)
				buf[2 + i] = (vcom[2 * i] << 4) | vcom[2 * i + 1];
	...
	case 2:
		...
		buf[0] = data_len;
		buf[1] = offset;
		ret = read_tcon_bin_data_by_id(bin_buf, data_size, TCON_B0_SPI);
		if (ret) {
			ALOGE("%s. can't read tcon_b0_spi_bin data", __func__);
			return -1;
		}
...
int handle_panel_ini(void)
{
	...
	lcd_buf_len = sizeof(struct lcd_attr_s) + sizeof(struct lcd_v2_attr_s);
	lcd_buf = (unsigned char *)malloc(lcd_buf_len);
	if (!lcd_buf) {
	...
	memset((void *)lcd_buf, 0, lcd_buf_len);
	...
	if (parse_panel_ini(file_name, lcd_buf, lcd_ext_attr, &bl_attr, &misc_attr, tcon_spi) < 0) {
	...
	if (check_param_valid(0, gLcdDataCnt, lcd_buf, tmp_len, tmp_buf) == CC_PARAM_CHECK_ERROR_NEED_UPDATE_PARAM) {
...
const char *get_model_sum_path_ex(void)
{
	...
	if (idme_get_var_external(MODEL_CONFIG_FILE_PATH, model_config_file, sizeof(model_config_file)) == 0) {
		...
		if (usb_inited == NULL || strcmp(usb_inited, "1")) {
			...
			ret = 0;	/* usb 0 is initialized */
		}
		if (ret == 0 && (!run_command("fatload usb 0 $loadaddr FAC_BOOT_SMART.cvt; ", 0) ||
				!run_command("fatload usb 0 $loadaddr FAC_BOOT_CVTE_LVDS.cvt; ", 0) ||
				!run_command("fatload usb 0 $loadaddr VbyOne.his; ", 0))) {
			memset(model_config_file_vb1, 0x00, sizeof(model_config_file_vb1));
			strncpy(model_config_file_vb1, model_config_file, (strlen(model_config_file) - strlen(".ini")));
			strcat(model_config_file_vb1, "_vb1.ini");
			printf("[%s, %d] diag model_config_file_vb1:%s\r\n", __FUNCTION__, __LINE__, model_config_file_vb1);
			return model_config_file_vb1;
		} else {
			printf("[%s, %d] diag model_config_file:%s\r\n", __FUNCTION__, __LINE__, model_config_file);
			return model_config_file;
		}
	}
	else
...
int handle_model_sum(void)
{
	...
	printf("use default model_name from env: %s\n", model);
	ret = parse_model_sum(get_model_sum_path(), model);
#else
	printf("use default model name: %s\n", MODEL_NAME_DEFAULT);
	if(idme_boot_mode() == IDME_BOOTMODE_DIAG) {
		ret = parse_model_sum(get_model_sum_path_ex(), MODEL_NAME_DEFAULT);
	} else {
		ret = parse_model_sum(get_model_sum_path(), MODEL_NAME_DEFAULT);
	}
#endif
#else
	model = getenv("model_name");
	...
	if (!idme_get_var_external("model_name", buf, sizeof(buf))) {
		...
		ret = parse_model_sum(get_model_sum_path(), buf);

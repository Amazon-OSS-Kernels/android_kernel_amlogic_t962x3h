...
#if defined(UBOOT_TARGET_PRODUCT_NAME_PRIMROSE)
static u32 fb_width;
...
void main_loop(void)
{
	...
	if (is_transition_bootmode) {
		if (!is_transition_done) {
...
#ifndef CONFIG_DEVICE_PRODUCT_HAZEL
			...
			if (ret) {
				...
				if (strstr(oem_data, "hazel-tm") != NULL) {	//hazel-tm transition
...
#elif defined(UBOOT_TARGET_PRODUCT_NAME_PRIMROSE)
			if (bootmode == IDME_BOOTMODE_TRANSITION) {
				...
				watchdog_disable();
				printf("close watchdog in case diag  \n");
				while (1)
					udelay(1000*1000);
				}
#else
	...
	if (break_command()) {
		...
		*/
		...
		if(amzn_target_is_lockdown()){
			printf("target is lockdown , watchdog enable !!\n");

		}else {
			watchdog_disable();
			printf("close watchdog in case debugging \n");
		}

...
static void lcd_module_enable(char *mode)
{
    ...
    LCDPR("lcd_module_enable1: lcd_status=%d\n", lcd_drv->lcd_status);
	LCDPR("lcd_module_enable1: lcd_init_level=%d\n", boot_ctrl.lcd_init_level);
...
(aml_lcd_driver.lcd_config->lcd_control.p2p_config->p2p_type & 0xff) << 8;
	...
	*/
	...
	/*set a new env variable, the kernel will use the variable in quiescent*/
	/// sprintf(lcd_boot_ctrl, "0x%08x", value | 0x80000);
	printf("lcd_ctrl_quiescent=%s\n", lcd_boot_ctrl);

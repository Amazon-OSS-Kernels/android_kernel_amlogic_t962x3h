...
static unsigned int update_bl_off_policy_str_env(void)
{
	char *bl_off_str = NULL;
	unsigned int bl_off_value = BL_OFF_POLICY_NONE;

	/* get bl_off_policy */
	bl_off_str = getenv("bl_off");
	if (bl_off_str == NULL) {
		LCDPR("bl: can't bl off policy, use default BL_OFF_POLICY_NONE\n");
		return bl_off_value;
	}

	if (strncmp(bl_off_str, "none", 2) == 0)
		bl_off_value = BL_OFF_POLICY_NONE;
	else if (strncmp(bl_off_str, "always", 2) == 0)
		bl_off_value = BL_OFF_POLICY_ALWAYS;
	else if (strncmp(bl_off_str, "once", 2) == 0)
		bl_off_value = BL_OFF_POLICY_ONCE;

	printf("bl: bl_off_value: %s\n", bl_off_str);
	return bl_off_value;
}
...
void aml_bl_power_ctrl(int status, int delay_flag)
{
	...
	if (status) {
		bl_off_policy = update_bl_off_policy_str_env();
		/* bl_off_policy */
		if (bl_off_policy != BL_OFF_POLICY_NONE) {
			LCDPR("bl_off_policy=%d for bl_off\n", bl_off_policy);
			printf("bl_off_policy=%d for bl_off, don't power on bl\n", bl_off_policy);
			return;
		}
...
static int aml_bl_config_load_from_unifykey(char *dt_addr, struct bl_config_s *bconf)
{
	...
	bconf->name[sizeof(bconf->name) - 1] = '\0'; /* ensure string ending */

...
struct virtual_partition virtual_partition_table[] = {
	...
	VIRTUAL_PARTITION_ELEMENT(MMC_DTB_NAME,
				  DTB_OFFSET, DTB_SIZE * DTB_COPIES,
				  VP_FLAG_BASE_RSV),
...
sprintf(str,"ddr_auto_fast_boot_check 4");
printf("\nstr=%s\n",str);
run_command(str,0);
...
int find_virtual_partition_by_name (char *name, struct partitions *partition)
{
	...
	if (!vpart) {
		apt_err("can't find %s in virtual partition\n", name);
		return -2;
	}

	if (vpart->flag & VP_FLAG_BASE_RSV) {

...
{
	strncpy(response_str, "FAIL", sizeof("FAIL"));
...
{
	strncpy(response_str, "OKAY", sizeof("OKAY"));
	if (s)strncat(response_str, s, RESPONSE_LEN - 4 - 1) ;
	fastboot_tx_write_str(response_str);
}
...
char * get_hw_reverision(){
	...
	if (!idme_get_var_external("board_id", buf, sizeof(buf))){
		if (!strncmp(HVT_BOARD_ID, buf, strlen(HVT_BOARD_ID)) || !strncmp(HVT2_BOARD_ID,buf,strlen(HVT2_BOARD_ID))){
			return "HVT";
...
#define PROD_VAR_SIZE 32
		...
		strncat(response, ammo_pv, chars_left);
#endif
#endif
		...
		strncat(response, "false", chars_left);
	} else {
...
static void cb_flashing(struct usb_ep *ep, struct usb_request *req)
{
	...
	if (!lock_s) {
		...
		printf("lock state: %s\n", lock_s);
		strncpy(lock_d, lock_s, sizeof(lock_d));
		lock_d[sizeof(lock_d) - 1] = '/0';
        }

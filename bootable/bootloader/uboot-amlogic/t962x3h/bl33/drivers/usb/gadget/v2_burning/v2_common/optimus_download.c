...
static int optimus_download_bootloader_image(struct ImgBurnInfo* pDownInfo, u32 dataSzReceived, const u8* data)
{
    ...
    if (size > (4U<<20)) {
        DWN_ERR("uboot.bin size 0x%llx > 4M unsupported\n", size);
        return 0;
    }
...
int is_the_flash_first_burned(void)
{
    ...
    const char* s = get_logoparam_value_ex("logoparam.var.upgrade_step", true, false);
#else
...
#ifdef CONFIG_LOGOPARAM_ENABLE
	/* here need to save the value to logo*/
    set_logoparam_value_ex("logoparam.var.firstboot", "1", true);
    set_logoparam_value_ex("logoparam.var.upgrade_step", upgrade_step, true);
#endif

...
static int* _cur_percents;
#elif defined(CONFIG_AML_MINUI)
#include "../../../../display/minui/render_api.h"

#define _RE_DRAW_LINES_ADDR 	(OPTIMUS_DOWNLOAD_DISPLAY_BUF + (5<<20))
#define _RE_DRAW_LINES_BACKUP	(_RE_DRAW_LINES_ADDR + 512 * 1024) 
static struct re_draw_lines{
    ushort x;
    ushort y;
    char* str;
} *_reDrawLines;
char* _backupLines;
static int  _numBackupLines;
static unsigned long _screenUpdateTime;
static int* _cur_percents;

static int _minui_screen_update(const int forceUpdate)
{
    unsigned long ts = 0;
    int i = 0;

    ts = get_timer(_screenUpdateTime);
    if (ts < 400 && !forceUpdate)
        return 0;

    for (; i < _numBackupLines; ++i) {
        DWN_DBG("drawline: x/y/str, %d, %d, %s\n", _reDrawLines[i].x, _reDrawLines[i].y, _reDrawLines[i].str);
        screen_drawtextline(gr_sys_font(), _reDrawLines[i].x, _reDrawLines[i].y, _reDrawLines[i].str, 1);
    }
    screen_update();
    _screenUpdateTime = get_timer(0);
    DWN_DBG("screen_update time %ld\n\n", _screenUpdateTime);
    return 0;
}

static int lcd_drawchars (ushort x, ushort y, uchar *str, int count)
{
    if (x) {
        DWN_DBG("percents:%s, xy %d %d\n", str, x, y);
        return 0;
    }

    _reDrawLines[_numBackupLines].x = x;
    _reDrawLines[_numBackupLines].y = y;
    _reDrawLines[_numBackupLines].str = (char*)_backupLines;
    memcpy(_backupLines, str, count);
    _backupLines[count] = '\0';

    _backupLines += count + 1;
    ++_numBackupLines;
...
static void lcd_printf(const char *fmt, ...)
{
	...
	nextLine = buf;
	strsep(&nextLine, "\n");
	line_len = strnlen(buf, CONFIG_SYS_PBSIZE);
	snprintf(buf + line_len, CONFIG_SYS_PBSIZE - line_len, "...%02d%%",*_cur_percents);
	lcd_drawchars(0, font->char_height * _prLines++, (uchar*)buf, strlen(buf));
}
#else
...
#ifdef CONFIG_AML_MINUI
#define MAX_SURFACE_NUM 8
static struct {
	grsurface	*surface;
	ulong		bmpAddr;
} _surfaceList[MAX_SURFACE_NUM];
//replace bmp display with minui implement
static int _minui_bmp_display(ulong bmpAddr, int dx, int dy)
{
	static ulong _prevBmpAddr = 0;
	static grsurface* _curSurface = NULL;
	int ret = 0;

	if (_prevBmpAddr != bmpAddr) {
		/*
		 *if (_curSurface) {
		 *        DWN_MSG("free prev surface\n");
		 *        res_free_surface(_curSurface);
		 *}
		 */
		int i = 0;

		_curSurface = NULL;
		for(; i < MAX_SURFACE_NUM; ++i) {
			if (_surfaceList[i].surface == NULL)
				break;
			if (_surfaceList[i].bmpAddr == bmpAddr) {
				_curSurface = _surfaceList[i].surface;
				break;
			}
		}

		if (!_curSurface) {
			DWN_MSG("New surface for bmp 0x%lx\n", bmpAddr);
			ret = res_create_display_surface_from_addr(bmpAddr, &_curSurface);
			if (ret < 0) {
				DWN_ERR("Fail get surface for bmp 0x%lx\n", bmpAddr);
				return -__LINE__;
			}
			_surfaceList[i].surface = _curSurface;
			_surfaceList[i].bmpAddr = bmpAddr;
		}
		_prevBmpAddr = bmpAddr;
	}

	DWN_DBG("surface_disaplay x/y %d/%d\n", dx, dy);
	surface_disaplay(_curSurface, 0, 0, dx, dy);
	/*_minui_screen_update(0);*/
	return 0;
}
#endif//#ifdef CONFIG_AML_MINUI
...
int video_res_prepare_for_upgrade(HIMAGE hImg)
{
...
#if defined(CONFIG_AML_MINUI)
    {
        ...
        _prLines = 0;
        _screenUpdateTime = get_timer(0);
        _backupLines = (char*)_RE_DRAW_LINES_BACKUP;
        _reDrawLines = (struct re_draw_lines *)_RE_DRAW_LINES_ADDR;
        _numBackupLines = 0;
        memset(&_surfaceList[0], 0, sizeof(_surfaceList));
    }
#else
    {
        const char* env_name  = _VIDEO_DEV_OPEN;

        WN_MSG("echo video prepare for upgrade:%s\n", env_name);
        ret = run_command(env_name, 0);
        if (ret)
        DWN_ERR("Fail in run_command[%s]\n", env_name);
   }
#endif//#if defined(CONFIG_AML_MINUI)
    return ret;
}
...
#ifdef CONFIG_AML_MINUI
    {
	bmp_header_t* bmpHeadInfo = (bmp_header_t*)bmpAddrEnv;
	unsigned display_width = getenv_ulong("fb_width", 0, 0) / 2 - bmpHeadInfo->width;
	unsigned display_height = getenv_ulong("fb_height", 0, 0) / 2 - bmpHeadInfo->height;

	if (strcmp(bmpOffsetName, "upgrade_upgrading_offset")) {
		screen_setcolor(0);
		screen_fillrect(display_width, display_height, bmpHeadInfo->width, bmpHeadInfo->height);
		screen_update();
		screen_setcolor(0xffffff00);
		_minui_bmp_display(bmpAddrEnv, display_width, display_height);
		_minui_screen_update(1);
	} else {
		_minui_bmp_display(bmpAddrEnv, display_width, display_height);
	}
...
//For total
    ...
    int         progressBarX_f;//record the initial x for minui, which need redraw by app
                                    ...
                                    int display_width,  int progressBarY_f )
{
    ...
    pUiProgress->progressBarX_f		     = pUiProgress->nextProgressBarX;
    ...
    _cur_percents = &(pUiProgress->curPercent);
...
int optimus_progress_ui_set_unfocus_bkg(__hdle hUiProgress, unsigned long unfocusBmpAddr)
{
    ...
    {
#ifdef CONFIG_AML_MINUI
	_minui_bmp_display(unfocusBmpAddr, progressBarX, progressBarY);
#else
        char cmd[64];
...
int optimus_progress_ui_direct_update_progress(__hdle hUiProgress, const int percents)
{
    ...
    int     nProgressBar= (nPercents * pUiProgress->nProgressBarOnePercent_f) / pUiProgress->progressBarWidth_f;
    int   i = 0;
...
#else
    int progressBarX          = pUiProgress->progressBarX_f;

    nProgressBar = percents * pUiProgress->nProgressBarOnePercent_f;
    nProgressBar /= pUiProgress->progressBarWidth_f;
    DWN_DBG("nProgressBar %d, progressBarX %d\n", nProgressBar, progressBarX);
    if (percents != 100) {
        show_logo_to_report_burning();
        optimus_progress_ui_set_unfocus_bkg(hUiProgress, getenv_hex("upgrade_unfocus_offset", 0));
    } else {
        show_logo_to_report_burn_success();
}
    for (i=0; i < nProgressBar; ++i, progressBarX += pUiProgress->progressBarWidth_f)
    {
        const int progressBarY = pUiProgress->progressBarY_f;
        const unsigned bmpAddr = pUiProgress->bmpAddr_f;

        _minui_bmp_display(bmpAddr, progressBarX, progressBarY);
    }
    _minui_screen_update(percents == 100);
#endif//#ifndef CONFIG_AML_MINUI
#ifdef CONFIG_OSD_SCALE_ENABLE
    //run_command("bmp scale", 0);
#endif// #ifdef CONFIG_OSD_SCALE_ENABLE

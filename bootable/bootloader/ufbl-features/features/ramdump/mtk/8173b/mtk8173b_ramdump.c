...
static void store_header(unsigned long long offset, unsigned char reason, const struct mdump_bank* banks)
{
	unsigned int idx;
...
int ramdump_in_ram_compress(void)
{
	...
	store_header(0, reason, s_mdump_banks);
...
void ramdump_get_mem_range(unsigned int *start, unsigned int *size)
{
	...
	*start = compress_req.compress_to_phyaddr;

...
function exec_build_kernel {
    if [ ! -e "${PLATFORM_EXTRACT_DIR}/device/amazon/common/verity/amazon_verity.x509.pem" ]
    then
        echo "${PLATFORM_EXTRACT_DIR}/device/amazon/common/verity/amazon_verity.x509.pem not found"
        exit 10
    fi
    mkdir -p "${WORKSPACE_OUT_DIR}/certs"
    cat "${PLATFORM_EXTRACT_DIR}/device/amazon/common/verity/amazon_verity.x509.pem" > "${WORKSPACE_OUT_DIR}/certs/amazon_verity.x509.pem"

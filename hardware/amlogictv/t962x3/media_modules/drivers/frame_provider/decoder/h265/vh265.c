...
static irqreturn_t vh265_isr_thread_fn(int irq, void *data)
{
	...
	if (hevc->eos || atomic_read(&hevc->inrun_flag) > 0)
		return IRQ_HANDLED;
	if (
#ifdef MULTI_INSTANCE_SUPPORT
		(!hevc->m_ins_flag) &&
...
static void timeout_process(struct hevc_state_s *hevc)
{
	...
	if (work_pending(&hevc->work) ||
		...
		pr_err("%s work pending or busy, do nothing.\n",__func__);
		return;
	}
	...
	void (*callback)(struct vdec_s *, void *), void *arg)
{
	...
	/* This block avoid there is still some work running */
	atomic_set(&hevc->inrun_flag, 1);
	while ((atomic_read(&vdec->inirq_thread_flag) > 0))
		schedule();
	cancel_work_sync(&hevc->work);
	cancel_work_sync(&hevc->timeout_work);

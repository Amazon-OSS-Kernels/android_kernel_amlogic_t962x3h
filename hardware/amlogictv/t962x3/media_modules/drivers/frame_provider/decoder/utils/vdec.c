...
static int enable_secure_di = 1;
static int enable_tunnel_di = 1;
...
s32 vdec_init(struct vdec_s *vdec, int is_4k)
{
	...
	if (p->use_vfm_path) {
		...
		if (strncmp("disable", vfm_path, strlen("disable"))) {
			...
			if (vdec_secure(vdec)) {
				snprintf(vdec->vfm_map_chain, VDEC_MAP_NAME_SIZE,
					"%s amlvideo %s amvideo", vdec->vf_provider_name,
					enable_secure_di ? "deinterlace" : "");
			} else {
				snprintf(vdec->vfm_map_chain, VDEC_MAP_NAME_SIZE,
					"%s %s", vdec->vf_provider_name,
			...
			snprintf(vdec->vfm_map_chain, VDEC_MAP_NAME_SIZE,
				"%s %s amvideo", vdec->vf_provider_name,
				enable_tunnel_di ? "deinterlace" : "");
			snprintf(vdec->vfm_map_id, VDEC_MAP_NAME_SIZE,
				"vdec-map-%d", vdec->id);
...
static void vdec_connect_list_force_clear(struct vdec_core_s *core, struct vdec_s *v_ref)
{
	...
	list_for_each_entry_safe(vdec, tmp,
		...
		if ((vdec->status == VDEC_STATUS_DISCONNECTED) &&
		    ...
		    pr_info("#1 active_mask= %ld   sched_mask= %ld \n",v_ref->active_mask,core->sched_mask);
			...
			if (core->last_vdec == v_ref)
				core->last_vdec = NULL;
			list_del(&vdec->list);
            pr_info("#2 active_mask= %ld   sched_mask= %ld \n",v_ref->active_mask,core->sched_mask);
		}
	}
...
module_param(enable_secure_di, int, 0664);
module_param(enable_tunnel_di, int, 0664);
/*

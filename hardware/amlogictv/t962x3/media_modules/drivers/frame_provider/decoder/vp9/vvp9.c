...
static irqreturn_t vvp9_isr_thread_fn(int irq, void *data)
{
	...
	if ((pbi->max_pic_h != 0) && (pbi->max_pic_w != 0) &&
			(pbi->max_pic_h < vp9_param.p.height ||
			pbi->max_pic_w < vp9_param.p.width)) {
			pbi->fatal_error |= DECODER_FATAL_ERROR_SIZE_OVERFLOW;
			return IRQ_HANDLED;
	}
...
static int ammvdec_vp9_probe(struct platform_device *pdev)
{
	...
	if ((debug & IGNORE_PARAM_FROM_CONFIG) == 0 &&
		...
		if (get_config_int(pdata->config, "HDRStaticInfo",
			...
			if (get_config_int(pdata->config,
				"matrixCoeffs", &config_val) == 0)
				pbi->video_signal_type |= (config_val << 0);
			else
				pbi->video_signal_type |= (9 << 0);	/* 2020 */
			if (get_config_int(pdata->config,
				"transfer", &config_val) == 0)
				pbi->video_signal_type |= (config_val << 8);
			else
				pbi->video_signal_type |= (16 << 8);	/* 2084 */
			if (get_config_int(pdata->config,
				"primaries", &config_val) == 0)
				pbi->video_signal_type |= (config_val << 16);
			else
				pbi->video_signal_type |= (9 << 16);	/* 2020 */
			if (get_config_int(pdata->config,
				"range", &config_val) == 0)
				pbi->video_signal_type |= (config_val << 25);
			else
				pbi->video_signal_type |= (0 << 25);	/* limit */
		}
		pbi->vf_dp = vf_dp;
	} else
#endif

...
static unsigned int dap_drc_control =
(GBL_COMP_LINE << DRC_MODE_BIT) |
(100 << DRC_HIGH_CUT_BIT) | (100 << DRC_LOW_BST_BIT);
...
static ssize_t dap_drc_control_show(struct class *cla,
				    struct class_attribute *attr, char *buf)
{
	static const char * const drcmode[] = {
		"CUSTOM_0", "CUSTOM_1", "LINE", "RF" };
	...
	pr_info("\tdap drc mode : %s\n", drcmode[dap_drc_control & 0x3]);
	pr_info("\tdap drc high cut scale : %d%%\n",
		(dap_drc_control >> DRC_HIGH_CUT_BIT) & 0xff);
	pr_info("\tdap drc low boost scale : %d%%\n",
		(dap_drc_control >> DRC_LOW_BST_BIT) & 0xff);
	pbuf += sprintf(pbuf, "%d\n", dap_drc_control);

	return pbuf - buf;
}

static ssize_t dap_drc_control_store(struct class *class,
				     struct class_attribute *attr,
				     const char *buf, size_t count)
{
	char tmpbuf[128];
	...
	if (strncmp(tmpbuf, "drcmode", 7) == 0) {
		if (kstrtoint(buf + i + 1, 16, &val))
			pr_info("kstrtoint err %s\n", __func__);
		val = val & 0x3;
		pr_info("dap drc mode set to %s\n", drcmode[val]);
		dap_drc_control = (dap_drc_control & (~3)) | val;
	} else if (strncmp(tmpbuf, "drchighcutscale", 15) == 0) {
		if (kstrtoint(buf + i + 1, 16, &val))
			pr_info("kstrtoint err %s\n", __func__);
		val = val & 0xff;
		pr_info("dap drc high cut scale set to %d%%\n", val);
		dap_drc_control =
		    (dap_drc_control & (~(0xff << DRC_HIGH_CUT_BIT))) |
		    (val << DRC_HIGH_CUT_BIT);
	} else if (strncmp(tmpbuf, "drclowboostscale", 16) == 0) {
		if (kstrtoint(buf + i + 1, 16, &val))
			pr_info("kstrtoint err %s\n", __func__);
		val = val & 0xff;
		pr_info("dap drc low boost scale set to %d%%\n", val);
		dap_drc_control =
		    (dap_drc_control & (~(0xff << DRC_LOW_BST_BIT))) |
		    (val << DRC_LOW_BST_BIT);
	} else
		pr_info("invalid args\n");
	return count;
...
static struct class_attribute audiodsp_attrs[] = {
	...
	__ATTR(dap_drc_control, 0664,
	       dap_drc_control_show, dap_drc_control_store),

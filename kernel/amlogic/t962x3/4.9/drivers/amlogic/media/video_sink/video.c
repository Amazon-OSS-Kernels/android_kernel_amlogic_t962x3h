...
static void vsync_toggle_frame(struct vframe_s *vf, int line)
{
	...
	if (debug_flag & DEBUG_FLAG_PRINT_DISPLAY_TIME) {
		...
		dis_time = current_time + vsync_duration;
		pr_info("toogle: index=%d, pts=%lld, dis_time=%lld\n",
			vf->omx_index, vf->pts_us64, dis_time);
	}
	...
	if (debug_flag & DEBUG_FLAG_PRINT_TOGGLE_FRAME) {
		...
		u64 mtime = ktime_get_ns();  // monotonic time ns

		pr_info("%s pts:%d.%06d pcr:%d.%06d vpts:%d.%06d apts:%d.%06d mtime:%lld\n",
			__func__, (vf->pts) / 90000,
			((vf->pts) % 90000) * 1000 / 90, (pcr) / 90000,
			((pcr) % 90000) * 1000 / 90, (vpts) / 90000,
			((vpts) % 90000) * 1000 / 90, (apts) / 90000,
			((apts) % 90000) * 1000 / 90, func_div(mtime, 1000));
	}
	...
	if (debug_flag & DEBUG_FLAG_HEMI_AVSYNC_DEBUG)
...
int hdmi_in_start_check(struct vframe_s *vf)
{
	...
	/*-16 or +16 ? select the one that closer to hdmin_delay_duration*/
...
#endif
{
	...
	provider_name = vf_get_provider_name(
		vd1_path_id == VFM_PATH_PIP ? RECEIVERPIP_NAME : RECEIVER_NAME);
	while (provider_name) {
		if (!vf_get_provider_name(provider_name))
			break;
		provider_name =
	...
	vf = video_vf_peek();
	/* abc123 HDMI-IN AV SYNC Control */
...
static u32 get_video_display_latency(void)
{
	...
	else if (video_inuse || omx_secret_mode)

 ...
 * layer
 */
struct vm_struct *cma_vm_area = NULL;
...
static void update_cma_vm_size(void)
{
	unsigned long tmp;

	tmp = get_cma_allocated() - ion_cma_allocated;
	cma_vm_area->size = tmp * PAGE_SIZE;
	cma_vm_area->nr_pages = tmp;
}
...
static int __init aml_cma_init(void)
{
	...
	cma_vm_area = __get_vm_area(PAGE_SIZE, VM_ALLOC | VM_NO_GUARD,
				    VMALLOC_START, VMALLOC_END);
	pr_info("%s, cma_vm_area:%p, addr:%p, size:%ld\n",
		__func__, cma_vm_area, cma_vm_area->addr, cma_vm_area->size);

	return 0;
}
pure_initcall(aml_cma_init);

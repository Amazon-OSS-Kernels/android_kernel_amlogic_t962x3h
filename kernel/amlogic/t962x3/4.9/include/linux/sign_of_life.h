...
typedef struct sign_of_life_ops {
	...
	int (*read_thermal_shutdown_reason)
		(life_cycle_reason_t *thermal_shutdown_reason);
 ...
 * set the boot reason which causing device booting
 */
 ...
 * Description: this function will
 * set the Shutdown reason which causing device shutdown
 */
int life_cycle_set_shutdown_reason(life_cycle_reason_t shutdown_reason);

/*
 * life_cycle_set_thermal_shutdown_reason
 * Description: this function will set the
 * Thermal Shutdown reason which causing device shutdown
 */
 ...
 * Description: this function will set the
 * special mode which causing device life cycle change
 */

...
static u32 stream_level, buf_latency;
				...
				const char *buf, size_t count)
{
	if (strncmp(buf, "decoded_frames", 14) == 0) {
		...
		if (kstrtoint(buf + 17, 10, (uint32_t *)&ch_configuration))
			return -EINVAL;
	} else if (strncmp(buf, "stream_level", 12) == 0) {
		if (kstrtoint(buf + 13, 10, (uint32_t *)&stream_level))
			return -EINVAL;
	} else if (strncmp(buf, "buf_latency", 11) == 0) {
		if (kstrtoint(buf + 12, 10, (uint32_t *)&buf_latency))
			return -EINVAL;
	}
	return count;
}
				      ...
				      struct class_attribute *attr, char *buf)
{
	...
	pos += sprintf(buf + pos, "stream_level:%d ", stream_level);
	pos += sprintf(buf + pos, "buf_latency:%d ", buf_latency);
	return pos;
}

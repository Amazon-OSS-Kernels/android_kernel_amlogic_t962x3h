/*
 * drivers/amlogic/clk/t5/t5_clk-pll.c
 *
...
static unsigned long meson_t5_pll_recalc_rate(struct clk_hw *hw,
					      unsigned long parent_rate)
{
	...
	if (p->width >= 2) {
		...
		if (!strcmp(clk_hw_get_name(hw), "t5_pcie_pll"))
...
static long meson_t5_pll_round_rate(struct clk_hw *hw, unsigned long rate,
				    unsigned long *parent_rate)
{
	struct meson_clk_pll *pll = to_meson_clk_pll(hw);
	...
	for (i = 0; i < pll->rate_count; i++) {
		if (rate <= rate_table[i].rate) {
			...
			if (!strcmp(clk_hw_get_name(hw), "t5_sys_pll") ||
			    !strcmp(clk_hw_get_name(hw), "t5_fixed_pll"))
...
static const struct pll_rate_table *meson_t5_get_pll_settings
...
static int meson_t5_pll_wait_lock(struct meson_clk_pll *pll,
				  struct parm *p_n)
{
	int delay = 24000000;
	u32 reg;
...
static int meson_t5_pll_set_rate(struct clk_hw *hw, unsigned long rate,
				 unsigned long parent_rate)
{
	struct meson_clk_pll *pll = to_meson_clk_pll(hw);
	struct parm *p;
	...
	rate_set = meson_t5_get_pll_settings(pll, rate);
	if (!rate_set)
		return -EINVAL;
	...
	if (readl(pll->base + p->reg_off) & MESON_PLL_ENABLE) {
		old_rate = meson_t5_pll_recalc_rate(hw, parent_rate);
		old_rate = meson_t5_pll_round_rate(hw, old_rate, NULL);
	...
	do {
		if (!strcmp(clk_hw_get_name(hw), "t5_sys_pll")) {
			...
			udelay(10);
		}  else if (!strcmp(clk_hw_get_name(hw), "t5_gp0_pll")) {
			writel((readl(cntlbase) | MESON_PLL_RESET)
			       & (~MESON_PLL_ENABLE), cntlbase);
			...
			writel(TL1_PLL_CNTL6,
			       cntlbase + (unsigned long)(6 * 4));
			udelay(10);
		} else if (!strcmp(clk_hw_get_name(hw), "t5_hifi_pll")) {
			writel((readl(cntlbase) | MESON_PLL_RESET)
			       & (~MESON_PLL_ENABLE), cntlbase);
			...
			writel(TL1_PLL_CNTL6,
			       cntlbase + (unsigned long)(6 * 4));
			udelay(10);
		} else if (!strcmp(clk_hw_get_name(hw), "t5_pcie_pll")) {
			...
			writel(TM2_PCIE_PLL_CNTL2_,
			       cntlbase + (unsigned long)(7 * 4));
		} else if (!strcmp(clk_hw_get_name(hw), "t5_fixed_pll")) {
			writel((readl(cntlbase) | MESON_PLL_RESET)
			       & (~MESON_PLL_ENABLE), cntlbase);
			udelay(100);
	...
	writel(readl(pll->base + p->reg_off) & (~MESON_PLL_RESET),
	       pll->base + p->reg_off);
	ret = meson_t5_pll_wait_lock(pll, p);
...
OUT:
	...
	if (ret) {
		pr_warn("%s: pll did not lock, trying to lock rate %lu again\n",
			__func__, rate);
		meson_t5_pll_set_rate(hw, rate, parent_rate);
	}

	return ret;
}

static int meson_t5_pll_enable(struct clk_hw *hw)
{
	struct meson_clk_pll *pll = to_meson_clk_pll(hw);
	struct parm *p;
	int ret = 0;
	...
	if (!strcmp(clk_hw_get_name(hw), "t5_gp0_pll") ||
	    !strcmp(clk_hw_get_name(hw), "t5_hifi_pll") ||
	    !strcmp(clk_hw_get_name(hw), "t5_sys_pll") ||
	    !strcmp(clk_hw_get_name(hw), "t5_fixed_pll")) {
		void *cntlbase = pll->base + p->reg_off;

		if (readl(cntlbase + (unsigned long)(6 * 4)) == TL1_PLL_CNTL6)
			first_set = 0;
	}
	...
	if (first_set) {
		...
		rate = meson_t5_pll_recalc_rate(hw, clk_hw_get_rate(parent));
		rate = meson_t5_pll_round_rate(hw, rate, NULL);
	}

	if (pll->lock)
		spin_unlock_irqrestore(pll->lock, flags);

	ret = meson_t5_pll_set_rate(hw, rate, clk_hw_get_rate(parent));

	return ret;
}

static void meson_t5_pll_disable(struct clk_hw *hw)
{
	struct meson_clk_pll *pll = to_meson_clk_pll(hw);
	struct parm *p = &pll->n;
	unsigned long flags = 0;
	...
	if (!strcmp(clk_hw_get_name(hw), "t5_fixed_pll"))
...
const struct clk_ops meson_t5_pll_ops = {
	.recalc_rate	= meson_t5_pll_recalc_rate,
	.round_rate	= meson_t5_pll_round_rate,
	.set_rate	= meson_t5_pll_set_rate,
	.enable		= meson_t5_pll_enable,
	.disable	= meson_t5_pll_disable,
};

const struct clk_ops meson_t5_pll_ro_ops = {
	.recalc_rate	= meson_t5_pll_recalc_rate,
};

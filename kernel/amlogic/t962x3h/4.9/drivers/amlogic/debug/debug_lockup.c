...
static void *cpu_tasklet[CPU] = {NULL};
static void *cpu_timer[CPU] = {NULL};
...
unsigned long atf_fiq_cpu_status[CPU];
#endif
	...
	unsigned int irq)
{
	...
	if (t_isr[irq] * 100 >= 50 * t_total[irq]) {
		t_isr_tmp = t_isr[irq];
...
void notrace sirq_out_hook(unsigned int cpu, unsigned long long tin, void *p)
{
	...
	if (cpu_sirq[cpu] && tin && (tout > tin + sirq_thr) &&
		...
		t_diff = tout - tin;
		do_div(t_diff, ns2ms);
		pr_err("SIRQLong___ERR. sirq:%pf  tout-tin:%llu ms\n",
 			p, t_diff);
...
void notrace tasklet_in_hook(unsigned int cpu, unsigned long long *tin, void *p)
{
	cpu_tasklet[cpu] = p;
	*tin = sched_clock();
}

void notrace tasklet_out_hook(unsigned int cpu, unsigned long long tin, void *p)
{
	unsigned long long tout = sched_clock();
	unsigned long long t_diff;

	if (cpu_tasklet[cpu] && tin && (tout > tin + sirq_thr) &&
		!oops_in_progress) {
		t_diff = tout - tin;
		do_div(t_diff, ns2ms);
		pr_err("TaskletLong___ERR. tasklet:%pf  tout-tin:%llu ms\n",
			p, t_diff);
	}

	cpu_tasklet[cpu] = NULL;
}

void notrace timer_in_hook(unsigned int cpu, unsigned long long *tin, void *p)
{
	cpu_timer[cpu] = p;
	*tin = sched_clock();
}

void notrace timer_out_hook(unsigned int cpu, unsigned long long tin, void *p)
{
	unsigned long long tout = sched_clock();
	unsigned long long t_diff;

	if (cpu_timer[cpu] && tin && (tout > tin + sirq_thr) &&
		!oops_in_progress) {
		t_diff = tout - tin;
		do_div(t_diff, ns2ms);
		pr_err("TimerLong___ERR. timer:%pf  tout-tin:%llu ms\n",
			p, t_diff);
	}
...
void atf_fiq_debug_entry(void)
{
	...
	/* sp_el0 */
	task = (struct task_struct *)atf_regs.sp;
	...
	pr_info(" ------------[ cpu%d ]------------", cpu);
	show_regs(&regs);
	sched_show_task((struct task_struct *)atf_regs.sp);
	pr_info("\n");
	atf_fiq_cpu_status[cpu] = 1;
	while (1)
		;
#else
	int cpu = smp_processor_id();
...
void pr_lockup_info(int c)
{
	...
	if (atf_fiq_debug_virt_addr && fiq_check_en) {
		pr_info("detect CPU%d lockup\n", c);
		cur_cpu = smp_processor_id();
		for_each_online_cpu(cpu) {
			if (cpu == cur_cpu)
				continue;
			atf_fiq_cpu_status[cpu] = 0;
			status = 0;
			arm_smccc_smc(0x820000f1, 0x2, cpu,
				0, 0, 0, 0, 0, &res);
			timeout = USEC_PER_SEC;
			while (status == 0 && timeout--) {
				udelay(1);
				status = atf_fiq_cpu_status[cpu];
			}
		}
	}
	return;
...
#define ONE_LOOP asm("mov r0, r0\n":::"r0", "memory")
#elif defined CONFIG_ARM64
#define ONE_LOOP asm("mov x0, x0\n":::"x0", "memory")
#endif
...
static int cpu_speed_test(void)
{
	...
	if (end - start > UINT_MAX) {
		WARN(1, "cpu_speed_test() consume %llu ns\n", end - start);
		return 0;
	}
	...
	pr_debug("cpu_speed_test() delta_us=%u mhz=%d\n", delta / 1000, mhz);
...
#if (defined CONFIG_ARM64) || (defined CONFIG_ARM64_A32)
	pr_info("fiq_check_en:%d\n", fiq_check_en);
	if (fiq_check_en) {
		memset(&res, 0, sizeof(res));
		arm_smccc_smc(0x820000f1, 0x1,
			(unsigned long)atf_fiq_debug_entry,
				0, 0, 0, 0, 0, &res.smccc);
		...
		if (atf_fiq_debug_phy_addr == 0 ||
				atf_fiq_debug_phy_addr == 0xFFFFFFFF) {
			pr_info("invalid atf_buf_phy_addr\n");
			return 0;
		}

		pr_info("atf phy addr:%lx, size: %lx\n",
			(unsigned long)atf_fiq_debug_phy_addr,
		...
		if (pfn_valid(__phys_to_pfn(atf_fiq_debug_phy_addr)))
			...
			atf_fiq_debug_virt_addr =
			ioremap_cache(atf_fiq_debug_phy_addr,
							atf_fiq_debug_buf_size);

		pr_info("atf virt addr:%lx\n",
				(unsigned long)atf_fiq_debug_virt_addr);

		if (atf_fiq_debug_virt_addr)
			memset(atf_fiq_debug_virt_addr, 0,
				atf_fiq_debug_buf_size);
	}
#endif

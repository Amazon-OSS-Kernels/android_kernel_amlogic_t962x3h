...
#define LONG_PRESS_DET_THRESHOLD  80 /* 80 * 25ms = 2s */
...
static int meson_adc_kp_long_detect(struct meson_adc_kp *kp, int code)
{
	if (code == 0 && kp->keymap.flags == -1)
		return 0;
	if (kp->keymap.code != code &&
			(kp->keymap.flags == 0 || kp->keymap.flags == -1))
		return 0;
	...
	if (kp->keymap.timestamp == LONG_PRESS_DET_THRESHOLD) {
		...
		if (kp->keymap.lcode == kp->pwrkey_code)
...
static void meson_adc_kp_poll(struct work_struct *pwork)
{
	...
	if (meson_adc_kp_long_detect(kp, code)) {

...
#define BLASTER_NAME		"irblaster"
#define CLASS_BLASTER_NAME		"meson-irblaster"
...
static char jackdetect_flag = (char) 0;
static char jack_fault_detect_flag = (char) 0;
static char handlerinit = (char) 0;
#endif

static int debug_enable;
static DEFINE_MUTEX(blaster_mutex);
static struct aml_irblaster_dev *irblaster;
struct iio_channel *bid_chan;
				...
				unsigned long args)
{
	...
	case CONSUMERIR_TRANSMIT:
		...
		psize = psize > MAX_PLUSE ? MAX_PLUSE:psize;
...
static DEVICE_ATTR(plug, 0444, show_plug, NULL);
...
{
	struct aml_irblaster_dev *dev = (struct aml_irblaster_dev *)data;
...
static irqreturn_t ir_detect_handler(int irq, void *data)
{
	if(handlerinit){
		jackdetect_flag = 1;
		irblaster->plug_flag = (!gpiod_get_value(irblaster->irdetect_gp));
		if (irblaster->timer_debounce)
			mod_timer(&irblaster->timer,jiffies + msecs_to_jiffies(irblaster->timer_debounce));
	}
...
{
	struct aml_irblaster_dev *dev = container_of(work, struct aml_irblaster_dev, work_irdetect);
	char data[2][32];
	char *envp[3] = {
		[0] = data[0],
		[1] = data[1],
...
static int  aml_irblaster_probe(struct platform_device *pdev)
{
	...
	/*init and set gpio of enable_pin  */
	irblaster_dev->enable_pin = of_get_named_gpio(pdev->dev.of_node, "enable_pin", 0);
	if (!gpio_is_valid(irblaster_dev->enable_pin )) {
		pr_err("enable_pin gpio %d is not valid.\n", irblaster_dev->enable_pin);
		return -EINVAL;
	}
	ret = gpio_direction_output(irblaster_dev->enable_pin, 1);
	if (ret) {
		pr_err("enable_pin gpio failed to set  output.\n");
		return -EINVAL;
	}
	ret = devm_gpio_request(&pdev->dev, irblaster_dev->enable_pin, "enable_pin");
	if (ret) {
		pr_err("can't request  enable_pin gpio %d", irblaster_dev->enable_pin);
		return -ENOENT;
	}

	/*init and set gpio of enablebuck_pin  */
	irblaster_dev->enablebuck_pin = of_get_named_gpio(pdev->dev.of_node, "enablebuck_pin", 0);
	if (!gpio_is_valid(irblaster_dev->enablebuck_pin)) {
		pr_err("enable_pin gpio %d is not valid.\n", irblaster_dev->enablebuck_pin);
		return -EINVAL;
	}

	ret = gpio_direction_output(irblaster_dev->enablebuck_pin, 1);
	if (ret) {
		pr_err("enablebuck_pin gpio failed to set  output.\n");
		return -EINVAL;
	}

	ret = devm_gpio_request(&pdev->dev,irblaster_dev->enablebuck_pin, "enablebuck_pin");
	if (ret) {
		pr_err("can't request  enablebuck_pin gpio %d", irblaster_dev->enablebuck_pin);
		return -ENOENT;
	}
	...
	irblaster_dev->irdetect_gp = gpiod_get(irblaster_dev->dev, "irdetect", GPIOD_IN);
	if (IS_ERR(irblaster_dev->irdetect_gp)) {
		pr_err("fail to claim ir detect gpio\n");
		goto err2;
	}
	// wait gpio OK.
	msleep(2);
	irblaster_dev->plug_flag = (!gpiod_get_value(irblaster_dev->irdetect_gp));
	irblaster_dev->timer_debounce = IR_DETECT_DEBOUNCE;
	irblaster_dev->dev = &pdev->dev;
	INIT_WORK(&(irblaster_dev->work_irdetect), work_irdetecting);
	setup_timer(&irblaster_dev->timer, gpio_tyrion_gpio_timer, (unsigned long) irblaster_dev);
/*
	PARSE_GPIO_NUM_PROP(irblaster_dev->dev.of_node, "irdetect", irblaster_dev->irdetect_gpio, 0);
	PARSE_AND_INIT_GPIO_IRQ(irblaster_dev->dev.of_node, "gpioirq", irblaster_dev->irdetect_gpioirq, irblaster_dev->irdetect_gpio, GPIO_IRQ_HIGH);
	PARSE_AND_INIT_GPIO_IRQ(irblaster_dev->dev.of_node, "removedetectirq", irblaster_dev->jackremove_gpioirq, irblaster_dev->irdetect_gpio, GPIO_IRQ_LOW);
*/

		/*request gpio irq to kernel*/
	irblaster_dev->irdetect_irq = gpiod_to_irq(irblaster_dev->irdetect_gp);
	ret = request_irq(irblaster_dev->irdetect_irq, ir_detect_handler, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "ir_detect_irq", NULL);
	if (ret) {
		pr_err("fail to request detect gpio irq\n");
		goto err2;
	}
//	PARSE_GPIO_NUM_PROP(pdev->dev.of_node, "irfaultdetect", irblaster_dev->irdetect_gpio, 0);
//	PARSE_AND_INIT_GPIO_IRQ(pdev->dev.of_node, "gpiofaultdetectirq", irblaster_dev->irdetect_gpioirq, irblaster_dev->irdetect_gpio, GPIO_IRQ_LOW);
	irblaster_dev->irftdetect_gp = gpiod_get(irblaster_dev->dev, "irfaultdetect", GPIOD_IN);
	if (IS_ERR(irblaster_dev->irftdetect_gp)) {
		pr_err("fail to claim ir fault gpio\n");
		goto err2;
	}
		/*request gpio irq to kernel*/
	irblaster_dev->irfaultdetect_gpioirq = gpiod_to_irq(irblaster_dev->irftdetect_gp);
	ret = request_irq(irblaster_dev->irfaultdetect_gpioirq, ir_fault_detect_handler, IRQF_TRIGGER_FALLING, "ir_fault_detect_irq", NULL);
	if (ret) {
		pr_err("fail to request fault gpio irq\n");
		goto err2;
	}

	pr_info("irdetect is %d, plug value is %d\n", irblaster_dev->irdetect_gpio, irblaster_dev->plug_flag);
	snprintf(data, sizeof(data), "plug=%d", irblaster_dev->plug_flag);
	pr_info("generate IR detect uevent %s\n", envp[0]);
	kobject_uevent_env(&irblaster_dev->dev->kobj, KOBJ_CHANGE, envp);

	irblaster_dev->carrier_freqs = DEFAULT_CARRIER_FREQ;
	irblaster_dev->duty_cycle = 100 - DEFAULT_DUTY_CYCLE;
	irblaster_dev->reg_base = reg_base;
	irblaster_dev->reset_base = reset_base;
	irblaster_dev->irq = res_irq->start;

	ret = alloc_chrdev_region(&irblaster_dev->chr_devno, 0,
...
err0:
	unregister_chrdev_region(irblaster_dev->chr_devno,
				BLASTER_DEVICE_COUNT);

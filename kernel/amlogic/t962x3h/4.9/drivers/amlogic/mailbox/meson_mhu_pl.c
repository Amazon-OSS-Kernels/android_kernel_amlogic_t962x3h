				  ...
				  size_t count, loff_t *ppos)
{
...
#ifdef CONFIG_AMLOGIC_HIFI4DSP
	if (!get_dsp_online_status(mbox_dev->char_name)) {
		dev_err(dev, "%s is not online.\n", mbox_dev->char_name);
		return -EFAULT;
	}
#endif
	...
	dev_dbg(dev, "%s %lx\n", __func__, (unsigned long)mbdata_async.complete);
				 ...
				 size_t count, loff_t *ppos)
{
	...
	list_for_each(list, &mbox_list[channel]) {
		...
		if (msg->task == current) {
			...
			ret = wait_for_completion_killable_timeout(&msg->complete, wait);

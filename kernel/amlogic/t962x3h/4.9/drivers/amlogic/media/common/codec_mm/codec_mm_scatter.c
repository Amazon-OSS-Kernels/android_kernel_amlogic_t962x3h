...
static int codec_mm_page_alloc_all_locked(
	struct codec_mm_scatter_mgt *smgt,
		phy_addr_type *pages, int num, int iscache, uint mem_flag)
{
	int alloced = 0;
	int can_from_scatter = iscache ? 0 : 1;
	int can_from_slot = 1;
	int new_alloc;
	int from_sys_first = mem_flag & CODEC_MM_FLAGS_SYS_FIRST;
	int can_from_sys = 1;

	while (alloced < num) {
		new_alloc = 0;
		if (can_from_scatter && !from_sys_first) {
			...
			if (new_alloc <= 0)
				can_from_scatter = 0;
		} else if (can_from_slot && !from_sys_first) {
			ATRACE_COUNTER("mmu alloc",
						MMU_ALLOC_from_slot);
			...
			if (new_alloc <= 0)
				can_from_slot = 0;
		} else if (!smgt->no_alloc_from_sys && !smgt->tvp_mode && can_from_sys) {
	...
	uint mem_flag)
{
	...
	smgt = codec_mm_get_scatter_mgt(mem_flag & CODEC_MM_FLAGS_TVP);
	startus = codec_mm_get_current_us();
	alloced_mms = NULL;
...
int codec_mm_scatter_cache_manage_switch = 1;
module_param(codec_mm_scatter_cache_manage_switch, uint, 0664);
MODULE_PARM_DESC(codec_mm_scatter_cache_manage_switch,
				"\n codec_mm_scatter_cache_manage on/off\n");
...
static void codec_mm_scatter_monitor(struct work_struct *work)
{
	...
	if (needretry)
		...
		codec_mm_schedule_delay_work(smgt, 100, 0);
	if (likely(codec_mm_scatter_cache_manage_switch))

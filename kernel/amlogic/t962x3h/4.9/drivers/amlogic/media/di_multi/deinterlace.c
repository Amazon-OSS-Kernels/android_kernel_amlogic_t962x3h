...
static int di_init_buf_simple(struct di_ch_s *pch)
{
	...
	/* local buf init */
	/**********************************************/
	tmp_meta = de_devp->local_meta_addr + ((de_devp->local_meta_size / DI_CHANNEL_NUB) * ch);
...
void dim_pre_de_process(unsigned int channel)
{
	...
	if (dim_is_slt_mode()) {
		if (DIM_IS_IC(T5) || DIM_IS_IC(T5DB) ||
		    DIM_IS_IC(T5D) || DIM_IS_IC_EF(SC2)) {
...
//ary 2020-12-09	ulong irq_flag2 = 0;
	...
	if (ppre->di_wr_buf) {
		...
		//check:
		ctimer = cur_to_usecs();
		if (((ctimer - ppre->irq_time[0]) >> 10) > dim_get_pre_tm_thd()) {
			PR_INF("pre_set timeout2:%lu,%lu,%llu\n",
			       (unsigned long)ppre->irq_time[0],
			       (unsigned long)ppre->irq_time[1],
			       ctimer);
		}
...
void dim_irq_pre(void)
{
	...
	/* check timeout*/
	ppre->irq_time[1] = cur_to_usecs();
	ctime = ppre->irq_time[1] - ppre->irq_time[0];

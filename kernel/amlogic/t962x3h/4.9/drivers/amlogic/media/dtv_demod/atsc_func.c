...
extern struct aml_demod_sta demod_status;
...
static void cci_run_new_hazel(struct amldtvdemod_device_s *devp)
{
...
static void cci_run_new_new(struct amldtvdemod_device_s *devp)
{
...
{
	if (isHazel())
		return cci_run_new_hazel(devp);
	else
		return cci_run_new_new(devp);
}
...
void set_cr_ck_rate_new(struct amldtvdemod_device_s *devp)
{
	...
	if (tuner_find_by_name(&devp->frontend, "r842") &&

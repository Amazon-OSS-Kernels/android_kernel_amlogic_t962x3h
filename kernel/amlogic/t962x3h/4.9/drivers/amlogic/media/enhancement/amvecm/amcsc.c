...
static int force_lumin_en;
module_param(force_lumin_en, int, 0664);
MODULE_PARM_DESC(force_lumin_en, "\n force_lumin_en\n");
	...
	enum vd_path_e vd_path)
{
	...
	if ((video_process_status[vd_path] == HDR_MODULE_BYPASS) &&
		...
		if ((is_video_layer_on(vd_path) ||
			...
			pr_csc(
4, "%d:set video_process_status[%s] = HDR_MODULE_ON\n",
__LINE__, vd_path == VD1_PATH ? "VD1" : "VD2");
	...
	if ((get_cpu_type() >= MESON_CPU_MAJOR_ID_G12A) &&
	    (signal_change_flag & (SIG_CS_CHG | SIG_RANGE_CHG | SIG_BS_CHG))) {
		if (!vinfo_lcd_support()) {
			mtx_setting(POST2_MTX, MATRIX_NULL, MTX_OFF);
		} else {
			pr_csc(12, "%s %d:vf %p,csc_type=%d,cur csc[vd%d]=%d\n",
			       __func__, __LINE__, vf, csc_type,
			       vd_path + 1, cur_csc_type[vd_path]);
			if (vf && vf->type & VIDTYPE_RGB_444 &&
			    (source_format[vd_path] == HDRTYPE_SDR) &&
			    (get_hdr_module_status(vd_path) != HDR_MODULE_OFF)) {
				pr_csc(12, "%s %d: type[vd%d]=%d\n",
					__func__, __LINE__,
					vd_path + 1,
	...
	enum vd_path_e vd_path)
{
	...
	pr_csc(16, "amvecm process vd%d %s %p %p, %d, %d\n",
	       ...
	       vf, vf_rpt, is_dolby_vision_on(), dovi_on);
	...
	if (cur_hdr10_tm_enable != hdr10_tm_enable) {
		/*hdr10_tm_enable change refresh*/
		pr_info("hdr10_tm_enable changed\n");
		cur_hdr10_tm_enable = hdr10_tm_enable;
		video_process_flags[vd_path] |= PROC_FLAG_FORCE_PROCESS;
	}

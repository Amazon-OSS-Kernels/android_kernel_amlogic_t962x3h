		...
		unsigned int cmd, unsigned long arg)
{
	...
	case AMVECM_IOC_S_HDR_TM:
		...
		hdr_tone_mapping_get(
			...
			hdr_tm);
		force_toggle();
		pr_amvecm_dbg("hdr tone mapping done\n");
		break;
	case AMVECM_IOC_G_HDR10_TM_STATE:
		if (copy_to_user(
			(void __user *)arg, &hdr10_tm_enable,
			sizeof(enum hdr10_tm_state)))
			ret = -EFAULT;
		pr_amvecm_dbg("hdr10+ tone mapping get done\n");
		break;
	case AMVECM_IOC_S_HDR10_TM_STATE:
		if (copy_from_user(
			&hdr10_tm_enable, (void __user *)arg,
			sizeof(enum hdr10_tm_state)))
			ret = -EFAULT;
		force_tog = 1;
		force_toggle();
		pr_amvecm_dbg("hdr10+ tone mapping done\n");
		break;
			...
			struct class_attribute *attr, char *buf)
{
	...
	unsigned char bit_depth = 12;
	unsigned int probe_color, probe_color1;
	...
	if (bit_depth == 10) {
		...
		len += sprintf(buf + len,
			"VPP_MATRIX_PROBE_COLOR %d, %d, %d\n",
			(((val2 & 0xf) << 8) | ((val1 >> 24) & 0xff)) >> 2,
			((val1 >> 12) & 0xfff) >> 2, (val1 & 0xfff) >> 2);
	}
...
static void force_dst_primary_refresh(void)
{
	vecm_latch_flag |= FLAG_COLORPRI_LATCH;
	force_toggle();
}
		...
		const char *buf, size_t count)
{
	...
	if (!strncmp(parm[0], "vpp_size", 8))
		...
		mltcast_skip_en = val;
		pr_info("setting value: %d\n", mltcast_skip_en);
	} else if (!strcmp(parm[0], "force_dst_primary")) {
		if (!strcmp(parm[1], "refresh")) {
			force_dst_primary_refresh();
			pr_info("force_dst_primary refresh\n");
		}
	} else
		pr_info("unsupport cmd\n");
...
void hdr_hist_config_int(void)
{
	...
	if (get_cpu_type() != MESON_CPU_MAJOR_ID_T5 && !is_meson_t5d_cpu()) {
...
static const struct vecm_match_data_s vecm_dt_t5 = {
	.vlk_chip = vlock_chip_t5,/*vlock_chip_t5d same as t5*/
	.vlk_support = true,
	.vlk_new_fsm = 1,
	.vlk_hwver = vlock_hw_tm2verb,
	.vlk_phlock_en = true,

...
static char bls_ver[32] = "blue stretch ver: 20210121";
int bs_proc_en;
/*default parameters*/
/*
	...
	for (d0 = 0; d0 < 17; d0++) {
		for (d1 = 0; d1 < 17; d1++) {
			for (d2 = 0; d2 < 17; d2++) {
				...
				bls_param->bls_proc(yuv_o, yuv_in, bls_param);
...
void bls_par_show(void)
{
	...
	bls_param = get_bls_par();
	pr_info("blue_stretch_en = %d\n", bls_param->blue_stretch_en);
	pr_info("blue_stretch_cr_inc = %d\n",
		bls_param->blue_stretch_cr_inc);
	pr_info("blue_stretch_cb_inc = %d\n",
		bls_param->blue_stretch_cb_inc);
	pr_info("blue_stretch_gain = %d\n",
		bls_param->blue_stretch_gain);
	pr_info("blue_stretch_gain_cb4cr = %d\n",
		bls_param->blue_stretch_gain_cb4cr);
	pr_info("blue_stretch_error_crp = %d\n",
		bls_param->blue_stretch_error_crp);
	pr_info("blue_stretch_error_crp_inv = %d\n",
		bls_param->blue_stretch_error_crp_inv);
	pr_info("blue_stretch_error_crn = %d\n",
		bls_param->blue_stretch_error_crn);
	pr_info("blue_stretch_error_crn_inv = %d\n",
		bls_param->blue_stretch_error_crn_inv);
	pr_info("blue_stretch_error_cbp = %d\n",
		bls_param->blue_stretch_error_cbp);
	pr_info("blue_stretch_error_cbp_inv = %d\n",
		bls_param->blue_stretch_error_cbp_inv);
	pr_info("blue_stretch_error_cbn = %d\n",
		bls_param->blue_stretch_error_cbn);
	pr_info("blue_stretch_error_cbn_inv = %d\n",
		bls_param->blue_stretch_error_cbn_inv);
	pr_info("blue_stretch_luma_high = %d\n",
...
int bls_par_dbg(char **parm)
{
	...
	if (!strcmp(parm[0], "bls_en")) {
		...
		//bs_proc_en = en;
		pr_info("en = %d\n", en);
	} else if (!strcmp(parm[0], "bls_fw_en")) {
		if (kstrtoul(parm[1], 10, &val) < 0)
			goto failed;
		bs_proc_en = (int)val;
		pr_info("bs_proc_en = %d\n", bs_proc_en);

		...
		u32 *avg_bitare)
{
	...
	/*calc timestamp from amstream buffer when pts reverse jumped*/
	if (pTable->last_checkin_pts < pTable->last_checkout_pts &&
		(timestampe_delayed < 10 || timestampe_delayed > (3 * 1000)) &&
		stbuf_level_cb && get_buf_by_type_cb) {
		diff2 = stbuf_level_cb(get_buf_by_type_cb(type));
	...
	if (likely((pTable->status == PTS_RUNNING) ||
		...
		if (pTable->status == PTS_LOADING) {
			if (tsync_get_debug_vpts() && (type == PTS_TYPE_VIDEO))
				pr_info("init vpts[%d] at 0x%x,0x%llx\n",
					type, val, rec->pts_uS64);

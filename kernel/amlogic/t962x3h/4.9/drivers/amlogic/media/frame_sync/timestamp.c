...
{
	u64 video_pts_64_local = 0;

	if (val > 0)
		video_pts_64_local = (u64)video_pts + (u64)val;
	else
		video_pts_64_local = (u64)video_pts - 1;

	if (video_pts_64_local > 0xffffffff)
		video_pts = (u32)(video_pts_64_local & 0xffffffff);
	else if (video_pts_64_local == 0xffffffff)
		video_pts = 0;
	else
		video_pts = video_pts + val;
...
void timestamp_pcrscr_inc(s32 inc)
{
	...
	pcrscr_u64 = (u64)system_time;

	if (system_time_up) {
#ifdef MODIFY_TIMESTAMP_INC_WITH_PLL
		inc = inc * timestamp_inc_factor / PLL_FACTOR;
#endif
		pcrscr_u64 = pcrscr_u64 + (u64)inc + (u64)system_time_inc_adj;
		if (pcrscr_u64 >= 0xffffffff)
			system_time = (u32)(pcrscr_u64 & 0xffffffff);
		else
...
void timestamp_pcrscr_inc_scale(s32 inc, u32 base)
{
	...
	if (system_time_up) {
		u32 r;
		temp = div_u64_rem(90000ULL * inc, base, &r) +
			system_time_inc_adj;

		pcrscr_u64 = pcrscr_u64 + (u64)temp;
		if (pcrscr_u64 >= 0xffffffff) {
			system_time = (u32)(pcrscr_u64 & 0xffffffff);
			video_pts = 0;
		} else {
...
void timestamp_need_videodrop_set(u32 need)
{
	need_videodrop = need;
	pr_info("set need_videodrop = %x\n", need);
}
EXPORT_SYMBOL(timestamp_need_videodrop_set);

u32 timestamp_need_videodrop_get(void)
{
	//pr_info("get need_videodrop = %x\n", need_videodrop);
	return need_videodrop;
}
EXPORT_SYMBOL(timestamp_need_videodrop_get);

...
int register_tsync_callbackfunc(enum tysnc_func_type_e ntype, void *pfunc)
{
	if (ntype >= TSYNC_FUNC_TYPE_MAX) {
		tsync_dbg(0, "register_tync_func ntype is err.\n");
		return -1;
	}
	switch (ntype) {
...
static void tsync_pcr_recover_with_video(void)
{
	...
	if (vb_level < (vb_size >> PCR_DETECT_MARGIN_SHIFT_VIDEO_LO)) {
		...
		tsync_dbg(0, " timestamp_pcrscr_set_adj(-%d);\n",
				PCR_RECOVER_PCR_ADJ);
...
static int tsync_mode_switch(int mode, unsigned long diff_pts, int jump_pts)
{
	...
	if (mode == 'T') {	/*D/A--> ... */
		if (tsync_av_mode == TSYNC_STATE_D) {
			...
			diff_pts < tsync_av_threshold_min) {
				tsync_dbg(0, "-wcs-< tsync_av_threshold_min--\n");
				tsync_av_mode = TSYNC_STATE_S;
				tsync_mode = TSYNC_MODE_AMASTER;
				tsync_av_latest_switch_time_ms = jiffies_ms;
				tsync_av_dynamic_duration_ms = 0;
			...
			diff_pts < tsync_av_threshold_max) {
				tsync_dbg(0, "-wcs-< tsync_av_threshold_max--\n");
		...
		if (tsync_mode != old_tsync_mode
			...
			tsync_dbg(0, "mode changes:tsync_mode:%c->%c,state:%c->%c,",
				 VA[old_tsync_mode], VA[tsync_mode],
				 old_tsync_av_mode, tsync_av_mode);
			tsync_dbg(0, "debugcnt=0x%x,diff_pts=%lu\n",
				debugcnt,
				diff_pts);
		return 0;
	}
	...
	tsync_dbg(0, "discontinue-tsync_mode:%c->%c,state:%c->%c,jiffies=%x",
			VA[old_tsync_mode], VA[tsync_mode],
			old_tsync_av_mode, tsync_av_mode,
			(u32)jiffies_ms32);
	tsync_dbg(0, "debugcnt=0x%x,diff_pts=%lu,tsync_mode=%d\n",
...
void tsync_avevent_locked(enum avevent_e event, u32 param)
{
	...
	//threshold 120s is an arbitrary value
		...
		if (tsync_enable && !get_vsync_pts_inc_mode()) {
			...
			tsync_mode = TSYNC_MODE_VMASTER;
			tsync_dbg(LOG_INFO, "tsync_mode=%d %d\n",
				TSYNC_MODE_VMASTER, __LINE__);
			tsync_dbg(LOG_INFO, "tsync_enable %d inc_mode %d\n",
				tsync_enable, get_vsync_pts_inc_mode());
		...
		tsync_dbg(LOG_INFO,
			"vpts to scr, apts: 0x%x, vpts: 0x%x stat: %d\n",
	...
	case VIDEO_STOP:
		...
		timestamp_need_videodrop_set(0);
		...
		if (tsdemux_pcrscr_valid_cb && tsdemux_pcrscr_valid_cb() == 1) {
			if (abs(param - oldpts) > tsync_av_threshold_min) {
				...
				if (apts_discontinue == 1) {
					apts_discontinue = 0;
					vpts_discontinue = 0;
					tsync_dbg(0, "set apts->pcrsrc,pcrsrc %x to %x,diff %d\n",
						timestamp_pcrscr_get(),
						timestamp_apts_get(),
						timestamp_apts_get()
						- timestamp_pcrscr_get());
					...
					tsync_dbg(0, "set para->pcrsrc,pcrsrc %x to %x,diff %d\n",
		...
		if (tsdemux_pcrscr_valid_cb && tsdemux_pcrscr_valid_cb() == 1) {
			if (abs(param - oldpts) > tsync_av_threshold_min) {
				...
				if (vpts_discontinue == 1) {
					tsync_dbg(0, "set para->pcrsrc,from %x to %x,diff %d\n",
						timestamp_pcrscr_get(), param,
						param-timestamp_pcrscr_get());
					apts_discontinue = 0;
					vpts_discontinue = 0;
					...
					tsync_dbg(0, "set vpts->pcrsrc,from %x to %x,diff %d\n",
						timestamp_pcrscr_get(),
						timestamp_vpts_get(),
						timestamp_vpts_get()
						- timestamp_pcrscr_get());
	...
	case AUDIO_START:
		...
		tsync_dbg(LOG_INFO, "audio start, reset apts = 0x%x\n",
					param);
		...
		tsync_dbg(LOG_INFO, "apts reset scr = 0x%x\n", param);
	...
	case VIDEO_PAUSE:
		...
		if (param == 1) {
			timestamp_pcrscr_enable(0);
			tsync_dbg(LOG_INFO, "video pause!\n");
		} else {
			if ((!apause_flag) || (!tsync_enable)) {
				timestamp_pcrscr_enable(1);
				tsync_dbg(LOG_INFO, "video resume\n");
			}
		}
		break;
...
void tsync_avevent(enum avevent_e event, u32 param)
{
	...
	tsync_dbg(LOG_INFO, "[%s]event:%d, param %d\n",
		...
		const char *buf, size_t size)
{
	...
	timestamp_vpts_set_u64((u64)pts);
	return size;
}
...
static ssize_t show_log_level(struct class *class,
			       struct class_attribute *attr, char *buf)
{
		...
		const char *buf, size_t size)
{
	...
	tsync_dbg(0, "[%s]tsync_mode=%d, buf=%s\n", __func__, tsync_mode, buf);
	return size;
}
...
static ssize_t store_checkin_firstvpts(struct class *class,
		struct class_attribute *attr,
		const char *buf, size_t size)
{
	unsigned int pts;
	...
	if (r != 0)
		return -EINVAL;
	timestamp_checkin_firstvpts_set(pts);
	return size;
}
...
static ssize_t store_log_level(struct class *class,
	struct class_attribute *attr,
	const char *buf, size_t size)
{
	int level = 0;
...
static ssize_t show_need_videodrop(struct class *class,
		struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", timestamp_need_videodrop_get());

}


static ssize_t store_need_videodrop(struct class *class,
		struct class_attribute *attr,
		const char *buf, size_t size)
{
	unsigned int need;
	...
	/*r = sscanf(buf, "%d", &need);*/
	r = kstrtoint(buf, 0, &need);
	if (r != 0)
		return -EINVAL;

	timestamp_need_videodrop_set(need);
	return size;
}
static struct class_attribute tsync_class_attrs[] = {
	__ATTR(pts_video, 0664, show_vpts, store_vpts),
	__ATTR(pts_video_u64, 0664, show_vpts_u64, store_vpts),
	__ATTR(pts_audio, 0664, show_apts, store_apts),
	__ATTR(pts_audio_u64, 0664, show_apts_u64, store_apts),
	...
	NULL),
	__ATTR(last_checkin_vpts, 0644, show_last_checkin_vpts,
	NULL),
	...
	__ATTR(log_level, 0664, show_log_level, store_log_level),
	__ATTR(pts_drop_video, 0644, show_videodrop_pts,
	NULL),
	__ATTR(need_drop_video, 0664, show_need_videodrop,
	store_need_videodrop),
	__ATTR_NULL
};
...
int tsync_store_fun(const char *trigger, int id, const char *buf, int size)
{
	...
	case 21:return store_checkin_firstvpts(NULL, NULL, buf, size);
	case 22:return store_vpts(NULL, NULL, buf, size);
	case 23:return store_apts(NULL, NULL, buf, size);
	case 24:return store_pcrscr(NULL, NULL, buf, size);
	case 25:return store_log_level(NULL, NULL, buf, size);
	case 27:return store_need_videodrop(NULL, NULL, buf, size);
	default:
		ret = -1;
	}
	return size;
...
int tsync_show_fun(const char *trigger, int id, char *sbuf, int size)
{
	...
	case 24:
		...
		break;
	case 25:
		ret = show_log_level(NULL, NULL, buf);
		break;
	case 26:
		ret = show_videodrop_pts(NULL, NULL, buf);
		break;
	case 27:
		ret = show_need_videodrop(NULL, NULL, buf);
		break;
	case 28:
...
static struct mconfig tsync_configs[] = {
	...
	MC_FUN_ID("checkin_firstvpts", tsync_show_fun, tsync_store_fun, 21),
	MC_FUN_ID("pts_video_u64", tsync_show_fun, tsync_store_fun, 22),
	MC_FUN_ID("pts_audio_u64", tsync_show_fun, tsync_store_fun, 23),
	MC_FUN_ID("pts_pcrscr_u64", tsync_show_fun, tsync_store_fun, 24),
	MC_FUN_ID("log_level", tsync_show_fun, tsync_store_fun, 25),
	MC_FUN_ID("pts_drop_video", tsync_show_fun, NULL, 26),
	MC_FUN_ID("need_drop_video", tsync_show_fun, tsync_store_fun, 27),
	MC_FUN_ID("last_checkin_vpts", tsync_show_fun, NULL, 28),
};
...
static int __init tsync_module_init(void)
{
	...
	if (r) {
		tsync_dbg(LOG_ERR, "tsync class create fail.\n");
		return r;
	}

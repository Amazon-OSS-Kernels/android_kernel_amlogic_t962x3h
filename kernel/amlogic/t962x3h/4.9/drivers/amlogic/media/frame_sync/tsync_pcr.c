...
/*debug parameters*/
#define TSYNC_PCR_DEBUG_CHECKINPTS   0x100
/*
...
static u32 tsync_firstcheckin_vpts;
static u32 tsync_firstcheckin_apts;
...
static u32 tsync_pcr_debug_count;
static u32 tsync_pcr_init_forced;
...
int get_stream_buffer_level(int type)
{
	if (type == 0) {
		/* video */
		if (get_buf_by_type_cb && stbuf_level_cb)
			return stbuf_level_cb(
			get_buf_by_type_cb(PTS_TYPE_VIDEO));
	} else {
		/* audio */
		if (get_buf_by_type_cb && stbuf_level_cb)
			return stbuf_level_cb(
			get_buf_by_type_cb(PTS_TYPE_AUDIO));
		}
...
int get_stream_buffer_size(int type)
{
	if (type == 0) {
		/* video */
		if (get_buf_by_type_cb && stbuf_size_cb)
			return stbuf_size_cb(
			get_buf_by_type_cb(PTS_TYPE_VIDEO));
	} else {
		/* audio */
		if (get_buf_by_type_cb && stbuf_size_cb)
...
static void tsync_set_pcr_mode(int mode, u32 param)
{
	...
	if (mode == 0) {
		...
		timestamp_vpts_set_u64((u64)param);
	} else if (mode == 1) {
...
static u32 ab_diff(u32 a, u32 b)
{
	if (a > b)
		return (u32)(a - b);
	else
...
static u32 tsync_calcpcr_by_latency(u32 dmx_pcr)
{
	struct tsync_pcr_s *priv = &tsync_s;
	u32 ref_pcr = -1, dmx_vpts, dmx_apts;
	int diff = 0, latency = 0;

	dmx_vpts = get_last_checkin_pts(PTS_TYPE_VIDEO);
	dmx_apts = get_last_checkin_pts(PTS_TYPE_AUDIO);
	if (VALID_PTS32(dmx_vpts) && VALID_PTS32(dmx_apts)) {
		if (dmx_vpts > dmx_apts)
			ref_pcr = dmx_apts;
		else
			ref_pcr = dmx_vpts;
	} else if (VALID_PTS32(dmx_vpts))
		ref_pcr = dmx_vpts;
	else if (VALID_PTS32(dmx_apts))
		ref_pcr = dmx_vpts;
	if (VALID_PTS32(ref_pcr) && VALID_PTS32(dmx_pcr)) {
		ref_pcr -= tsync_pcr_ref_latency;
		diff = dmx_pcr - ref_pcr;
		if (diff < 200 * 90)
			latency = 200 * 90;
		else if (diff > TIME_UNIT90K)
			latency = TIME_UNIT90K;
		else
			latency = diff;
		timestamp_set_pcrlatency(latency);
		priv->av_pcr_latency = latency;
	}
	pr_info("%s, ref_pcr %x, diff %d, latency %d ms\n",
		__func__, ref_pcr, diff, latency / 90);
	return ref_pcr;
}
...
static u32 tsync_calcpcr_by_audio(u32 first_apts, uint64_t audio_cache_pts)
{
	...
	if (tsync_pcr_init_forced) {
		if (audio_cache_pts > TIME_UNIT90K)
			ref_pcr = first_apts + audio_cache_pts - TIME_UNIT90K;
		else
			ref_pcr = first_apts + audio_cache_pts
	...
	pr_info("tsync_calcpcr_by_audio ref_pcr %x\n", ref_pcr);
	return ref_pcr;
}
...
u32 tsync_pcr_get_ref_pcr(void)
{
	...
	pr_info("audio_cache_apts=0x%llx, video_cache_pts=0x%llx\n",
		audio_cache_pts, video_cache_pts);
...
void tsync_pcr_pcrscr_set(void)
{
	...
	if (first_apts && first_vpts)
		pr_info("\n%s start, first apts %x vpts %x, diff %d ms\n",
			__func__, first_apts, first_vpts,
			(int)(first_vpts - first_apts) / 90);
	else if (first_apts)
		pr_info("\n%s start, first apts %x\n",
			__func__, first_apts);
	else if (first_vpts)
		pr_info("\n%s start, first vpts %x\n",
			__func__, first_vpts);
	if (tsdemux_pcrvideo_valid_cb &&
		tsdemux_pcraudio_valid_cb && tsdemux_pcraudio_valid_cb()) {
		if (!VALID_PTS32(cur_checkin_apts))
	...
	/* if the AV were arrived and valid, set the status to "synced" */
	if (tsdemux_pcrvideo_valid_cb && tsdemux_pcraudio_valid_cb) {
		if (cur_checkin_apts && VALID_PTS32(cur_checkin_apts) &&
			VALID_PTS32(cur_checkin_vpts))
			priv->av_init_synced = (TSYNC_PCR_INITCHECK_VPTS |
				TSYNC_PCR_INITCHECK_APTS);
		else if (tsync_pcr_debug)
			pr_info("%s, av init is not synced\n", __func__);
	}
	...
	if (cur_pcr && !(tsync_pcr_inited_flag & complete_init_flag)
		...
		if ((abs(cur_pcr - cur_checkin_vpts) >
			...
			ref_pcr = tsync_pcr_get_ref_pcr();
			if (VALID_PTS32(cur_checkin_vpts) && !first_apts) {
				tsync_pcr_inited_mode = INIT_PRIORITY_VIDEO;
				tsync_pcr_inited_flag |=
					TSYNC_PCR_INITCHECK_VPTS;
			} else if (first_apts && first_vpts && (first_vpts <
				first_apts + MIN_SYNC_ACHACH_TIME ||
				(first_vpts - first_apts) >
				PLAY_PCR_INVALID_THRESHOLD)) {
				tsync_pcr_inited_mode = INIT_PRIORITY_VIDEO;
				tsync_pcr_inited_flag |=
					TSYNC_PCR_INITCHECK_VPTS;
			...
			ref_pcr = tsync_calcpcr_by_latency(cur_pcr);
...
static bool tsync_video_discontinue_check(u32 param)
{
	u32 pcrscr = timestamp_pcrscr_get();
	u32 checkin_vpts = (u32) get_last_checkin_pts(PTS_TYPE_VIDEO);
	u32 checkin_apts = (u32) get_last_checkin_pts(PTS_TYPE_AUDIO);
	u32 cur_vpts = timestamp_vpts_get();
	u32 buf_level = get_stream_buffer_level(0);
	u32 buf_size = get_stream_buffer_size(0);
	bool discontinued = false;

	if (tsync_pcr_inited_mode == INIT_PRIORITY_PCR)
		return discontinued;
	discontinued = true;
	if ((checkin_vpts > param && param > pcrscr &&
		param - pcrscr < PLAY_PCR_INVALID_THRESHOLD) ||
		(checkin_vpts > pcrscr && pcrscr > param)) {
		discontinued = false;
	}
	if (tsync_pcr_debug & 0x02) {
		pr_info("%s, %s param:%x, cur_vpts %x, pcrscr %x invpts %x\n",
			__func__, discontinued ? "discontinued" : "skiped",
			param, cur_vpts, pcrscr, checkin_vpts);
		pr_info("%s, diff %d %d %d ms, level %d size %d\n",
			__func__, (int)(checkin_vpts - param) / 90,
			(int)(checkin_vpts - pcrscr) / 90,
			(int)(checkin_vpts - checkin_apts) / 90,
			buf_level, buf_size);
	}
	return discontinued;
}
...
static void tsync_process_discontinue(void)
{
	...
	if (tsync_demux_last_pcr != 0 && cur_tsdemuxpcr != 0 &&
		...
		if (tsync_pcr_demux_pcr_used() == 0 &&
			ab_diff(cur_tsdemuxpcr, cur_vpts) <
			tsync_vpts_discontinuity_margin() &&
			tsync_demux_pcr_continue_count > 10 &&
			ab_diff(cur_checkin_vpts, cur_vpts) <
			tsync_vpts_discontinuity_margin() &&
	...
	if ((tsync_pcr_tsdemuxpcr_discontinue &
		...
		VIDEO_DISCONTINUE) {
		if (ab_diff(cur_checkin_vpts, cur_vpts) >
			tsync_vpts_discontinuity_margin()) {
...
void tsync_pcr_check_checinpts(void)
{
	...
	if (tsync_pcr_astart_flag == 1) {
		...
		if ((checkin_apts != 0xffffffff) &&
			...
			if (ab_diff(last_pcr_checkin_apts, checkin_apts) >
	...
	if (tsync_pcr_vstart_flag == 1) {
		...
		if ((checkin_vpts != 0xffffffff) &&
			...
			if (ab_diff(last_pcr_checkin_vpts, checkin_vpts)
				> PLAY_PCR_INVALID_THRESHOLD) {
				tsync_pcr_tsdemuxpcr_discontinue |=
				VIDEO_DISCONTINUE;
	...
	if (tsync_firstcheckin_apts == 0)
		tsync_firstcheckin_apts = timestamp_firstapts_get();
	if (tsync_firstcheckin_vpts == 0)
		tsync_firstcheckin_vpts = timestamp_checkin_firstvpts_get();

	if (checkin_vpts && checkin_apts &&
		checkin_apts != 0xffffffff && checkin_vpts != 0xffffffff &&
		(tsync_pcr_debug & TSYNC_PCR_DEBUG_CHECKINPTS) &&
		(((tsync_pcr_debug_count++) % 50) == 0)) {
		/*dump checkin a/v and pcr pts */
		cur_pcr = timestamp_pcrscr_get();
		pr_info("checkin vpts %x,apts %x,pcr %x;\n",
			checkin_vpts, checkin_apts, cur_pcr);
		pr_info("checkin pa %d, pv %d,av %d ms;\n",
			(int)(cur_pcr - checkin_apts) / 90,
			(int)(cur_pcr - checkin_vpts) / 90,
			(int)(checkin_apts - checkin_vpts) / 90);
	}
}
...
void tsync_pcr_avevent_locked(enum avevent_e event, u32 param)
{
	...
	case VIDEO_STOP:
		...
		wait_pcr_count = 0;
		pr_info("clear init flag when video stop\n");
	...
	case VIDEO_TSTAMP_DISCONTINUITY:{
		...
		if (tsync_pcr_inited_mode != INIT_PRIORITY_PCR) {
			if (!tsync_video_discontinue_check(param)) {
				timestamp_vpts_set(param);
				break;
			}
			tsync_pcr_discontinue_point = param;
			...
			/*
			 * currently, the 33rd bit(the highest bit) of
			 * the pts value was ignored in the tsync-pcr
			 * driver, there are some limitations that can
			 * not be handled in specific scenarios, such as
			 * the highest bit was being randomly changed
			 */
			if (tsync_pcr_demux_pcr_used() == 1) {
	...
	case AUDIO_PRE_START:
		...
		if (tsync_pcr_inited_flag) {
			/*
			 * if the pcrpts was initiliazed by V only,
			 * need to recheck the pcrpts when A comes;
			 */
			if (tsdemux_pcrvideo_valid_cb &&
				...
				tsync_pcr_pcrscr_set();
				if (tsync_pcr_inited_flag)
					pr_info("AUDIO PERSTART,apts later\n");
			}
			break;
		}
		if (video_pid_valid) {
...
static unsigned long tsync_pcr_check(void)
{
	...
	u32 cur_apts = 0;
	u32 cur_vpts = 0;
	u32 first_vpts, first_apts;
	u32 diff_apts = 0, diff_vpts = 0;
	...
	if ((!(tsync_pcr_inited_flag & TSYNC_PCR_INITCHECK_VPTS)) &&
		...
		if (last_checkin_vpts != 0xffffffff &&
			last_checkin_vpts > first_vpts)
			diff_vpts = last_checkin_vpts - first_vpts;
		if (last_checkin_apts != 0xffffffff &&
			last_checkin_apts > first_apts)
			diff_apts = last_checkin_apts - first_apts;
		if (last_checkin_vpts == 0xffffffff &&
			last_checkin_apts == 0xffffffff) {
			first_time_record = cur_system_time;
		} else if (((cur_system_time - first_time_record) < 270000 &&
			diff_apts < TIME_UNIT90K) && video_pid_valid) {
		} else {
			pr_info("%s, forced_init, video_valid=%d, diff_apts %x\n",
				__func__, video_pid_valid, diff_apts);
			tsync_pcr_init_forced = 1;
			if (video_pid_valid)
				tsync_pcr_inited_mode = INIT_PRIORITY_VIDEO;
			else
				tsync_pcr_inited_mode = INIT_PRIORITY_AUDIO;
...
static void tsync_pcr_param_reset(void)
{
	...
	tsync_video_continue_count = 0;
	last_discontinue_checkin_apts = 0;
	tsync_firstcheckin_apts = 0;
	tsync_firstcheckin_vpts = 0;
	tsync_pcr_init_forced = 0;
	last_discontinue_checkin_vpts = 0;
	tsync_video_discontinue = 0;
	video_pid_valid = false;
	video_jumped = false;
...
int tsync_pcr_start(void)
{
	...
	if (tsync_get_mode() == TSYNC_MODE_PCRMASTER) {
		...
		add_timer(&tsync_pcr_check_timer);
		/* set the default reference latency to 500 ms */
		tsync_pcr_ref_latency = (TIME_UNIT90K * 0.5);
	...
	tsync_demux_pcr_time_interval = 0;
	tsync_system_time_interval = 0;
	priv->av_init_synced = 0;
	priv->av_pcr_latency = 0;
	return 0;
}
...
static ssize_t tsync_firstcheckin_avstate_show(struct class *cla,
				      struct class_attribute *attr,
				      char *buf)
{
	int state = 0;

	/* 0,concurrently; 1,a recheckin; 2,v recheckin;*/
	if (tsync_firstcheckin_apts == 0 || tsync_firstcheckin_vpts == 0)
		state = 0;
	else if (tsync_firstcheckin_apts != timestamp_firstapts_get() &&
		tsync_firstcheckin_vpts == timestamp_checkin_firstvpts_get())
		state = 1;
	else if (tsync_firstcheckin_apts == timestamp_firstapts_get() &&
		tsync_firstcheckin_vpts != timestamp_checkin_firstvpts_get())
		...
		const char *buf, size_t count)
{
	...
	pr_info("%s(%d)\n", __func__, tsync_disable_demux_pcr);
	if (r != 0)
		return -EINVAL;
	return count;
}
...
static ssize_t tsync_video_level_show(struct class *cla,
		struct class_attribute *attr,
		char *buf)
{
	u32 video_level = 0;
...
static ssize_t tsync_last_discontinue_checkin_apts_show(struct class *cla,
		struct class_attribute *attr,
		char *buf)
{
	return sprintf(buf, "0x%x\n", last_discontinue_checkin_apts);
...
static struct class_attribute tsync_pcr_class_attrs[] = {
	...
	__ATTR(tsync_firstcheckin_avstate, 0644,
	       tsync_firstcheckin_avstate_show, NULL),
	...
	__ATTR(tsync_disable_demux_pcr, 0664,
	...
	__ATTR(tsync_enable_demuxpcr_check, 0644,
		tsync_enable_demuxpcr_check_show,
		tsync_enable_demuxpcr_check_store),
	__ATTR(tsync_enable_bufferlevel_tune, 0644,
		tsync_enable_bufferlevel_tune_show,
		tsync_enable_bufferlevel_tune_store),
	__ATTR(tsync_pcr_inited_flag, 0644, tsync_pcr_inited_flag_show, NULL),
	__ATTR(tsync_demux_pcr_valid, 0644, tsync_demux_pcr_valid_show, NULL),
	...
	__ATTR(tsync_pcr_latency_value, 0644,
		tsync_pcr_latency_value_show, tsync_pcr_latency_value_store),
	__ATTR(tsync_audio_underrun, 0644,
		tsync_audio_underrun_show, NULL),
	__ATTR(tsync_audio_underrun_maxgap, 0644,
		tsync_audio_underrun_maxgap_show,
		tsync_audio_underrun_maxgap_store),
	__ATTR_NULL};

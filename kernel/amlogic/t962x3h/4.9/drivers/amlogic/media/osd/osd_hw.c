...

{
	...
	if (index == OSD1) {
		...
		if ((blending->osd_blend_mode == OSD_BLEND_AC) ||
			...
			if (src_width == width) {
				/* corrected if reverse */
				if ((osd_hw.osd_reverse[index] == REVERSE_TRUE) ||
					(osd_hw.osd_reverse[index] == REVERSE_X)) {
					if (osd_hw.dst_data[index].x *
						blending->screen_ratio_w_den %
						blending->screen_ratio_w_num > 0)
						osd_hw.free_dst_data[index].x_start++;
				}
			}
			if (src_height == height) {
				/* corrected if reversed */
				if ((osd_hw.osd_reverse[index] == REVERSE_TRUE) ||
					(osd_hw.osd_reverse[index] == REVERSE_Y)) {
					if (osd_hw.dst_data[index].y *
						blending->screen_ratio_h_den %
						blending->screen_ratio_h_num > 0)
						osd_hw.free_dst_data[index].y_start++;
				}
			}
...
static int affinity_set_task(void *data)
{
	...
	while (affinity_info.run_affinity_task) {
		wait_for_completion_interruptible(&affinity_info.affinity_task_com);
		msleep(200);

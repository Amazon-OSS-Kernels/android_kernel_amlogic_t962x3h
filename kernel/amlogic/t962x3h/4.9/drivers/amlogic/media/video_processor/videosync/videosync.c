...
static const struct of_device_id videosync_dt_match[] = {
	{
...
static int videosync_drv_probe(struct platform_device *pdev)
{
	return 0;
}

static int videosync_drv_remove(struct platform_device *pdev)
{
	return 0;
}

#ifdef CONFIG_PM
static int videosync_drv_suspend(struct platform_device *pdev,
				 pm_message_t state)
{
...
static int videosync_drv_resume(struct platform_device *pdev)
{
	if (!IS_ERR_OR_NULL(vp_dev))
		complete(&vp_dev->thread_active);
	return 0;
...
static struct platform_driver videosync_drv = {
	.probe = videosync_drv_probe,
	.remove = videosync_drv_remove,
#ifdef CONFIG_PM
	.suspend = videosync_drv_suspend,
	.resume = videosync_drv_resume,
#endif
	.driver = {
		.name = "videosync",
		.owner = THIS_MODULE,
		.of_match_table = videosync_dt_match,
	}
};
...
static int __init videosync_init(void)
{
	...
	if (platform_driver_register(&videosync_drv))
		pr_err("Failed to register videosync driver\n");
...
static void __exit videosync_exit(void)
{
	...
	kfree(vp_dev);
	vp_dev = NULL;
	platform_driver_unregister(&videosync_drv);

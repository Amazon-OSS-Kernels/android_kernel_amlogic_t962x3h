...
#define DEBUG_TMP 0
#define SUPPORT_TUNNELMODE_VIDEODROP 1
...
static bool vf_busy;
static DEFINE_SPINLOCK(omx_set_pts);
...
#define M_PTS_SMOOTH_MAX 18000
#define M_PTS_SMOOTH_MIN 4500
#define M_PTS_SMOOTH_FACTOR 8
...
static u32 smooth_sync_enable = 1;
static u32 smooth_sync_expired;
...
static inline int video_vf_put(struct vframe_s *vf)
{
	...
	if (atomic_dec_and_test(&vf->use_cnt)) {
		ATRACE_COUNTER("put frame", (uintptr_t)vf);
		if (vf->type & VIDTYPE_DI_PW)
			di_put++;
			       ...
			       int toggled_cnt)
{
	...
	if (tsync_get_mode() == TSYNC_MODE_PCRMASTER &&
		smooth_sync_enable && !smooth_sync_expired) {

		if (next_vf->pts)
			org_vpts = next_vf->pts;
		else
		...
		if ((org_vpts + vsync_pts_inc - systime) <
			...
			expired = (int)(adjust_pts - org_vpts) >= 0;
			if (debug_flag & DEBUG_FLAG_PRINT_TOGGLE_FRAME) {
				pr_info("smooth_sync %d vpts:%d.%03d pcr:%d.%03d pts:%d.%03d\n",
				expired, (org_vpts) / 90000,
				((org_vpts) % 90000) / 90, (systime) / 90000,
				((systime) % 90000) / 90, (adjust_pts) / 90000,
				((adjust_pts) % 90000) / 90);
			}
			return expired;
		}
	...
	if (tsync_get_mode() == TSYNC_MODE_PCRMASTER) {
		...
		expired = (timestamp_pcrscr_get() + vsync_pts_align >= pts) ?
				true : false;
		if (debug_flag & DEBUG_FLAG_HDMI_AVSYNC_DEBUG)
			pr_info("vpts_expire, pcr %x, pts %x, expired %d\n",
				timestamp_pcrscr_get(), pts, expired);
	} else
...
static int update_amvideo_recycle_buffer(void)
{
	...
	if (recycle_cnt[0] &&
		...
		pr_err("expection: normal amvideo recycle_cnt:%d, put_cnt:%d, recycle_buf:%p-%p, to_put:%p, rdma_buf:%p!\n",
		       recycle_cnt[0], put_cnt, recycle_buf[0][0], recycle_buf[0][1],
	...
	if (!recycle_cnt[0]) {
		...
		for (i = 0; i < put_cnt; i++) {
			if (to_put_buf[i])
				recycle_buf[0][recycle_cnt[0]++] = to_put_buf[i];
			if (rdma_buf &&
		...
		pr_debug(
			...
			recycle_buf[0][2], recycle_buf[0][3], put_cnt, rdma_buf);
	} else {
		pr_debug(
...
static int update_pip_recycle_buffer(void)
{
	...
	if (recycle_cnt[1] &&
		...
		pr_err("expection: pip recycle_cnt:%d, put_cnt:%d, recycle_buf:%p-%p, to_put:%p, rdma_buf:%p!\n",
		       recycle_cnt[1], put_cnt, recycle_buf[1][0], recycle_buf[1][1],
		       to_put_buf[0], rdma_buf);
		return -1;
	}
	if (!recycle_cnt[1]) {
		...
		for (i = 0; i < put_cnt; i++) {
			if (to_put_buf[i])
				recycle_buf[1][recycle_cnt[1]++] = to_put_buf[i];
			if (rdma_buf &&
			    (rdma_buf == to_put_buf[i]))
				mismatch = false;
		}
		...
		pr_debug(
			...
			recycle_buf[1][2], recycle_buf[1][3], put_cnt, rdma_buf);
	} else {
		pr_debug(
...
#endif
{
	...
	#ifdef SUPPORT_TUNNELMODE_VIDEODROP
	if (timestamp_need_videodrop_get()) {
	...
	if ((recycle_cnt[0] > 0) && (cur_dispbuf != &vf_local)) {
		for (i = 0; i < recycle_cnt[0]; i++) {
			if (recycle_buf[0][i] &&
				...
				di_release_count++;
				if (recycle_buf[0][i]->type & VIDTYPE_DI_PW)
					di_release++;
	...
	if ((recycle_cnt[1] > 0) && (cur_pipbuf != &local_pip)) {
		for (i = 0; i < recycle_cnt[1]; i++) {
			if (recycle_buf[1][i] &&
				...
				if (recycle_buf[1][i]->type & VIDTYPE_DI_PW)
					di_release++;
	...
	if (gvideo_recv[0])
		gvideo_recv[0]->func->early_proc(gvideo_recv[0], over_sync ? 1 : 0);
	if (gvideo_recv[1])
		gvideo_recv[1]->func->early_proc(gvideo_recv[1], over_sync ? 1 : 0);
...
static void release_di_buffer(int inst)
{
	...
	for (i = 0; i < recycle_cnt[inst]; i++) {
		if (recycle_buf[inst][i] &&
			...
			di_release_count++;
			if (recycle_buf[inst][i]->type & VIDTYPE_DI_PW)
				di_release++;
...
static int video_receiver_event_fun(int type, void *data, void *private_data)
{
	if (type == VFRAME_EVENT_PROVIDER_UNREG) {
		...
		pr_info("unreg:di_get=%d,di_put=%d,di_release=%d, diff=%d\n",
			di_get, di_put, di_release,
			di_get - di_put - di_release);
		...
		pr_info("reg:di_get=%d,di_put=%d,di_release=%d, diff=%d\n",
			di_get, di_put, di_release,
			di_get - di_put - di_release);
		max_dis_hwc = 0;
		min_dis_hwc = 0;
...
static void set_omx_pts(u32 *p)
{
	...
	if (set_from_hwc == 1) {
		...
		if (vf_busy)
			pr_info("setpts: vf busy\n");

		spin_lock_irqsave(&omx_set_pts, flags);
		vf_busy = true;
		spin_unlock_irqrestore(&omx_set_pts, flags);
...
static ssize_t video_display_latency_show(struct class *cla,
				      struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", get_video_display_latency());
}

static ssize_t video_display_latency_store(struct class *cla,
				      struct class_attribute *attr,
				      const char *buf, size_t count)
{
	pr_info("no support set video display\n");
	return 0;
}
...
static struct class_attribute amvideo_class_attrs[] = {
	...
	__ATTR(over_sync_count,
	       ...
	       NULL),
	__ATTR(video_display_latency,
	       0664,
	       video_display_latency_show,
	       video_display_latency_store),
	__ATTR(pps_coefs,
	       0664,
	       NULL,
	       pps_coefs_store),
	...
	if (p_amvideo->cpu_type ==
		...
		if (legacy_vpp)
			...
			layer_cap =
				...
				LAYER0_SCALER |
				LAYER0_AVAIL;
		} else if (is_meson_tm2_cpu() && is_meson_rev_a()) {
			layer_cap =
				LAYER1_SCALER |
				LAYER1_AVAIL |
				LAYER0_AFBC |

	...
	if (setting->sc_v_enable) {
		...
		if (bit9_mode) {
			VSYNC_WR_MPEG_REG(
				...
				VPP_COEF_VERT |
				VPP_COEF_9BIT);
			for (i = 0; i < (vpp_filter->vpp_vert_coeff[1] & 0xff); i++) {
				VSYNC_WR_MPEG_REG(
					VPP_SCALE_COEF + misc_off,
...
{
	static u32 vd1_enabled, vd2_enabled, od_enabled = 0xff;
	...
	if (vd2_enabled != vd_layer[1].enabled) {
		vd2_enabled = vd_layer[1].enabled;
		enable_changed |= 1;
	}
	...
	if (enable_changed) {
		...
		if (vd_layer[0].enabled || vd_layer[1].enabled)
			...
			next_od_enable = 0;
		if (next_od_enable != od_enabled) {
			if (vd_layer[0].global_debug & DEBUG_FLAG_BASIC_INFO)
				pr_info("vpp blend: switch od-> %s\n",
					next_od_enable ? "on" : "off");
			data[0] = 0;
			data[1] = next_od_enable;
			aml_lcd_atomic_notifier_call_chain(LCD_EVENT_TCON_OD_ON_OFF, (void *)data);
			od_enabled = next_od_enable;
		}
	}
...
void dump_pps_coefs_info(u8 layer_id, u8 bit9_mode, u8 coef_type)
{
	...
	if (layer_id == 0) {
		...
		if (hscaler_8tap_enable[layer_id]) {
			if (bit9_mode) {
				if (coef_type == 0) {
					WRITE_VCBUS_REG(VPP_SCALE_COEF_IDX, 0x00004100 |
						VPP_COEF_9BIT);
					READ_VCBUS_REG(VPP_SCALE_COEF);
					for (i = 0; i < VPP_FILER_COEFS_NUM; i++)
						pr_info("VD1 8TAP 9bit HORZ coef[%d]=%x\n",
							i, READ_VCBUS_REG(VPP_SCALE_COEF));
					for (i = 0; i < VPP_FILER_COEFS_NUM; i++)
						pr_info("VD1 8TAP 9bit HORZ coef[%d]=%x\n",
							i, READ_VCBUS_REG(VPP_SCALE_COEF));

					WRITE_VCBUS_REG(VPP_SCALE_COEF_IDX, 0x00004180 |
						VPP_COEF_9BIT);
					READ_VCBUS_REG(VPP_SCALE_COEF);
					for (i = 0; i < VPP_FILER_COEFS_NUM; i++)
						pr_info("VD1 8TAP 9bit HORZ coef[%d]=%x\n",
					...
					WRITE_VCBUS_REG(VPP_SCALE_COEF_IDX, 0x00004000 |
						VPP_COEF_9BIT);
					READ_VCBUS_REG(VPP_SCALE_COEF);
					for (i = 0; i < VPP_FILER_COEFS_NUM; i++)

/*
 * drivers/amlogic/media/vin/adc/adc.c
 *
...
static unsigned int adc_afe_reg_read(unsigned int reg)
{
	if (!probe_finish || !adc_devp)
		return -1;

	return readl(adc_devp->afe_vir_addr + (reg << 2));
}
...
void adc_pll_down(void)
{
	...
	if (!adc_devp->pll_flg && get_cpu_type() >= MESON_CPU_MAJOR_ID_T5) {
		adc_hiu_wr_bits(HHI_VDAC_CNTL1_T5, 0, 3, 1);
		adc_hiu_wr_bits(HHI_DADC_CNTL3, 0, 0, 2);
		adc_hiu_wr_bits(HHI_ADC_PLL_CNTL0_TL1, 0, 28, 1);
	}

	if (!adc_devp->pll_flg && get_cpu_type() == MESON_CPU_MAJOR_ID_TM2) {
		HIU_WR(HHI_VDAC_CNTL1_T5, 0x00000008);
		HIU_WR(HHI_DADC_CNTL3, 0x0);
		HIU_WR(HHI_ADC_PLL_CNTL0_TL1, 0x20000000);
		HIU_WR(HHI_S2_DADC_CNTL2, 0x000f0fc0);
		adc_afe_reg_write(AFE_VAFE_CTRL0, 0x0);
		adc_afe_reg_write(AFE_VAFE_CTRL1, 0x0);
		adc_afe_reg_write(AFE_VAFE_CTRL2, 0x0);
	}
...
static void adc_dump_regs(void)
{
	...
	pr_info("AFE_VAFE_CTRL0(0x%x):0x%x\n",
		AFE_VAFE_CTRL0, adc_afe_reg_read(AFE_VAFE_CTRL0));
	pr_info("AFE_VAFE_CTRL1(0x%x):0x%x\n",
		AFE_VAFE_CTRL1, adc_afe_reg_read(AFE_VAFE_CTRL1));
	pr_info("AFE_VAFE_CTRL2(0x%x):0x%x\n",
		AFE_VAFE_CTRL2, adc_afe_reg_read(AFE_VAFE_CTRL2));
	pr_info("-------- hiu --------\n");
	pr_info("HHI_DADC_CNTL(0x%x):0x%x\n",
		HHI_DADC_CNTL, aml_read_hiubus(HHI_DADC_CNTL));
	pr_info("HHI_DADC_CNTL2(0x%x):0x%x\n",
		HHI_DADC_CNTL2, aml_read_hiubus(HHI_DADC_CNTL2));
	pr_info("HHI_DADC_CNTL3(0x%x):0x%x\n",
		HHI_DADC_CNTL3, aml_read_hiubus(HHI_DADC_CNTL3));
	pr_info("HHI_DADC_CNTL4(0x%x):0x%x\n",
		HHI_DADC_CNTL4, aml_read_hiubus(HHI_DADC_CNTL4));
	pr_info("HHI_S2_DADC_CNTL(0x%x):0x%x\n",
		HHI_S2_DADC_CNTL, aml_read_hiubus(HHI_S2_DADC_CNTL));
	pr_info("HHI_S2_DADC_CNTL2(0x%x):0x%x\n",
		HHI_S2_DADC_CNTL2, aml_read_hiubus(HHI_S2_DADC_CNTL2));
	pr_info("HHI_ADC_PLL_CNTL0_TL1(0x%x):0x%x\n",
		HHI_ADC_PLL_CNTL0_TL1, aml_read_hiubus(HHI_ADC_PLL_CNTL0_TL1));
	pr_info("HHI_ADC_PLL_CNTL1_TL1(0x%x):0x%x\n",
		HHI_ADC_PLL_CNTL1_TL1, aml_read_hiubus(HHI_ADC_PLL_CNTL1_TL1));
	pr_info("HHI_ADC_PLL_CNTL2_TL1(0x%x):0x%x\n",
		HHI_ADC_PLL_CNTL2_TL1, aml_read_hiubus(HHI_ADC_PLL_CNTL2_TL1));
	pr_info("HHI_ADC_PLL_CNTL3_TL1(0x%x):0x%x\n",
		HHI_ADC_PLL_CNTL3_TL1, aml_read_hiubus(HHI_ADC_PLL_CNTL3_TL1));
	pr_info("HHI_ADC_PLL_CNTL4_TL1(0x%x):0x%x\n",
		HHI_ADC_PLL_CNTL4_TL1, aml_read_hiubus(HHI_ADC_PLL_CNTL4_TL1));
	pr_info("HHI_ADC_PLL_CNTL5_TL1(0x%x):0x%x\n",
		HHI_ADC_PLL_CNTL5_TL1, aml_read_hiubus(HHI_ADC_PLL_CNTL5_TL1));
	pr_info("HHI_ADC_PLL_CNTL6_TL1(0x%x):0x%x\n",
		HHI_ADC_PLL_CNTL6_TL1, aml_read_hiubus(HHI_ADC_PLL_CNTL6_TL1));
	pr_info("HHI_VDAC_CNTL0_T5(0x%x):0x%x\n",
		HHI_VDAC_CNTL0_T5, aml_read_hiubus(HHI_VDAC_CNTL0_T5));
	pr_info("HHI_VDAC_CNTL1_T5(0x%x):0x%x\n",
		HHI_VDAC_CNTL1_T5, aml_read_hiubus(HHI_VDAC_CNTL1_T5));
	pr_info("-------- clk_ctrl --------\n");
	pr_info("HHI_DEMOD_CLK_CNTL1(0x%x):0x%x\n",
		HHI_DEMOD_CLK_CNTL1, dd_tvafe_hiu_reg_read(HHI_DEMOD_CLK_CNTL1));
}

static ssize_t adc_debug_store(struct class *cla,
			       ...
			       const char *buf, size_t count)
{
	...
	if (parm[0] && !strcmp(parm[0], "state")) {
		...
		if (!strcmp(parm[0], "vafe"))
			adc_afe_reg_write(reg_addr, reg_val);
		else if (!strcmp(parm[0], "hiu"))
			HIU_WR(reg_addr, reg_val);
		else if (!strcmp(parm[0], "clk_ctrl"))
			dd_tvafe_hiu_reg_write(reg_addr, reg_val);
		else
...
static ssize_t adc_debug_show(struct class *cla,
			      struct class_attribute *attr, char *buf)
{
	...
	pr_info("debug : %d\n", adc_devp->print_en);
	pr_info("echo dump_reg > /sys/class/adc/attr\n");
	pr_info("echo down > /sys/class/adc/attr\n");
	pr_info("echo vafe reg_addr reg_val >  /sys/class/adc/attr\n");
	pr_info("echo hiu  reg_addr reg_val >  /sys/class/adc/attr\n");
	pr_info("echo clk_ctrl  reg_addr reg_val >  /sys/class/adc/attr\n");
	pr_info("version : %s\n", TVDIN_ADC_VER);
	return 0;
}
...
static struct class_attribute adc_class_attr[] = {
	__ATTR(attr, 0664, adc_debug_show, adc_debug_store),
	__ATTR_NULL
};

static struct class adc_class = {
	.name = ADC_DEV_NAME,
	.class_attrs = adc_class_attr,
...
static void __exit adc_uninit(void)
{

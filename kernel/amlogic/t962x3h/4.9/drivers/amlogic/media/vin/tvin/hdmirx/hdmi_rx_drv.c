...
static ssize_t hdmirx_game_dev_show(struct device *dev,
	struct device_attribute *attr,
	char *buf)
{
...
static ssize_t rx_edid_with_port_show(struct device *dev,
	struct device_attribute *attr,
	char *buf)
{
	return 0;
...
static ssize_t rx_edid_with_port_store(struct device *dev,
	struct device_attribute *attr,
	const char *buf,
	size_t count)
{
				 ...
				 size_t count)
{
	...
	if (edid_delivery_mothed == EDID_DELIVERY_ONE_PORT)
		edid_select = 0;
	return count;
}
...
static DEVICE_ATTR(edid_with_port, 0644, rx_edid_with_port_show, rx_edid_with_port_store);
...
static int hdmirx_probe(struct platform_device *pdev)
{
	...
	ret = device_create_file(hdevp->dev, &dev_attr_game_dev);
	if (ret < 0) {
		rx_pr("hdmirx: fail to create game_dev file\n");
		goto fail_create_game_dev;
	}
...
fail_get_resource_irq:
	return ret;
fail_create_game_dev:
	device_remove_file(hdevp->dev, &dev_attr_game_dev);

...
#define ALLOW_LIST_SIZE 25
#define DEVICE_CNT 4
const u8 spd_allow_list[DEVICE_CNT][ALLOW_LIST_SIZE] = {
	/* ps5 */
...
#define DEVICE_CNT_SPEC 1
const u8 spd_list[DEVICE_CNT_SPEC][ALLOW_LIST_SIZE] = {
	/* Roku box */
	{0x4d, 0x0, 0x74, 0x0, 0x72, 0x0, 0x20, 0x0, 0x48, 0x0, 0x4d, 0x0, 0x20,
		0x0, 0x78, 0x0, 0x44, 0x0, 0x6d, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1},
};

int rx_get_specific_dev_num(void)
{
	int i;
	struct spd_infoframe_st *spdpkt;

	spdpkt = (struct spd_infoframe_st *)&(rx_pkt.spd_info);
	for (i = 0; i < DEVICE_CNT_SPEC; i++) {
		if (!memcmp((char *)spdpkt + 5, spd_list[i], ALLOW_LIST_SIZE)) {
			if (log_level & 0x1000)
				rx_pr("special dev num =%d\n", i);
			break;
		}
	}
	return i;
...
bool rx_is_specific_20_dev(void)
{
	...
	for (i = 0; i < DEVICE_CNT; i++) {
		if (!memcmp((char *)spdpkt + 5, spd_allow_list[i], ALLOW_LIST_SIZE)) {
			ret = true;
			if (log_level & 0x1000)
				rx_pr("white dev=%d\n", i);
		}

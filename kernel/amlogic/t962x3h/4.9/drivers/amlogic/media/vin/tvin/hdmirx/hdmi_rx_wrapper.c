...
static int hdcp_none_wait_max = 90;
...
static bool edid_ver_need_chg(void)
{
	bool flag = false;
...
static bool hdcp_sts_update(void)
{
	bool ret = false;
...
int special_dev_wait_cnt(void)
{
	int wait_cnt = 0;
	int dev_num = rx_get_specific_dev_num();

	if (dev_num == 0) //Roku box
		wait_cnt = 300;
	return wait_cnt;
}

bool is_unnormal_format(uint32_t wait_cnt)
{
	bool ret = false;
	...
	if (rx.hdcp.hdcp_version == HDCP_VER_NONE) {
		...
		if (dev_is_appletv_v2) {
			if (wait_cnt == hdcp_none_wait_max * 3)
				ret = false;
		...
		if (dev_is_appletv_v2) {
			...
			|| rx.cur.hdcp22_state)
				ret = false;
			if (log_level & VIDEO_LOG)
				rx_pr("apple tv hdcp none waiting\n");
		}
	}
...
void rx_get_open_port(unsigned int *open_info)
{
	*open_info = open_port;
	if (open_port)
		open_port = 0;
...
static bool high_bandwidth_cond(void)
{
	bool ret = false;

	if (rx.cur.vactive > 1080) {
		ret = true;
	}
	return ret;
}
		...
		if (rx_is_timing_stable()) {
			if (++sig_stable_cnt >= sig_stable_max) {
				...
				if (is_unnormal_format(sig_stable_cnt))
					break;
				/* wait for specail dev */
				if (special_dev_wait_cnt() > sig_stable_cnt)
					break;
				/* is game dev? */
				game_dev_flag = high_bandwidth_cond();
				rx_pr("game_dev_flag:%d\n", game_dev_flag);

...
static bool white_pattern_reset_pag(enum tvin_port_e port, struct tvafe_cvd2_s cvd2)
{
	if (IS_TVAFE_AVIN_SRC(port)) {
		...
		if (((av1_plugin_state == 0) || (av2_plugin_state == 0)) &&
			...
			tvafe_pr_info("av1:%u av2:%u\n", av1_plugin_state, av2_plugin_state);
			top_init_en = 0;
			return true;
		}
	}

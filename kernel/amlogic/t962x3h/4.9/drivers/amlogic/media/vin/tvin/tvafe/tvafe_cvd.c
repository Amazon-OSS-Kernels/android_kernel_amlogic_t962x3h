...
module_param(no_color_burst_cfg, uint, 0664);
MODULE_PARM_DESC(no_color_burst_cfg, "no_color_burst_cfg");
...
static unsigned int acd_ntscm_h_back = 0x00880358;
module_param(acd_ntscm_h_back, uint, 0644);
MODULE_PARM_DESC(acd_ntscm_h_back, "acd_ntscm_h_back test");
	...
	if (lines->check_cnt == user_param->vline_chk_cnt) {
		...
		if (tvafe_dbg_print & TVAFE_DBG_AUTO_VS)
			tvafe_pr_info("%s: lines->val:%d %d %d %d, l_ave:%d, 0xe6:%d, vs_adj_en:%d, level:%d dir:%d\n",
					...
					lines->val[3], l_ave, R_APB_REG(CVD2_REG_E6), cvd2->info.vs_adj_en,
					cvd2->info.vs_adj_level, cvd2->info.vs_adj_dir);
		...
		if (diff > hs_adj_th_level0) {
			...
			if (diff > hs_adj_th_level4) {
				...
				if (cvd2->info.hs_adj_dir) {
					h_dynamic_val = (diff - hs_adj_th_level4) / user_param->horizontal_stp1[4];
					acd_2d_adjust = h_dynamic_val + user_param->horizontal_dir1[4];
				} else {
					h_dynamic_val = (diff - hs_adj_th_level4) / user_param->horizontal_stp0[4];
					acd_2d_adjust = h_dynamic_val + user_param->horizontal_dir0[4];
				}
			} else if (diff > hs_adj_th_level3) {
				cvd2->info.hs_adj_level = 3;
				if (cvd2->info.hs_adj_dir) {
					h_dynamic_val = (diff - hs_adj_th_level3) / user_param->horizontal_stp1[3];
					acd_2d_adjust = h_dynamic_val + user_param->horizontal_dir1[3];
				} else {
					h_dynamic_val = (diff - hs_adj_th_level3) / user_param->horizontal_stp0[3];
					acd_2d_adjust = h_dynamic_val + user_param->horizontal_dir0[3];
				}
			} else if (diff > hs_adj_th_level2) {
				cvd2->info.hs_adj_level = 2;
				if (cvd2->info.hs_adj_dir) {
					h_dynamic_val = (diff - hs_adj_th_level2) / user_param->horizontal_stp1[2];
					acd_2d_adjust = h_dynamic_val + user_param->horizontal_dir1[2];
				} else {
					h_dynamic_val = (diff - hs_adj_th_level2) / user_param->horizontal_stp0[2];
					acd_2d_adjust = h_dynamic_val + user_param->horizontal_dir0[2];
				}
			} else if (diff > hs_adj_th_level1) {
				cvd2->info.hs_adj_level = 1;
				if (cvd2->info.hs_adj_dir) {
					h_dynamic_val = (diff - hs_adj_th_level1) / user_param->horizontal_stp1[1];
					acd_2d_adjust = h_dynamic_val + user_param->horizontal_dir1[1];
				} else {
					h_dynamic_val = (diff - hs_adj_th_level1) / user_param->horizontal_stp0[1];
					acd_2d_adjust = h_dynamic_val + user_param->horizontal_dir0[1];
				}
			} else {
				cvd2->info.hs_adj_level = 0;
				if (cvd2->info.hs_adj_dir) {
					h_dynamic_val = (diff - hs_adj_th_level0) / user_param->horizontal_stp1[0];
					acd_2d_adjust = h_dynamic_val + user_param->horizontal_dir1[0];
				} else {
					h_dynamic_val = (diff - hs_adj_th_level0) / user_param->horizontal_stp0[0];
					acd_2d_adjust = h_dynamic_val + user_param->horizontal_dir0[0];
				}
			}
			...
			if (cvd2->info.hs_adj_dir == 1) {
				...
				if (tvafe_dbg_print & TVAFE_DBG_AUTO_HS) {
					tvafe_pr_info("%s:hs_adj_dir:%d,lev:%x,h_dy:%x,0x2e:%x,0x12d:%x\n",
						__func__, cvd2->info.hs_adj_dir,cvd2->info.hs_adj_level,
						h_dynamic_val, R_APB_REG(CVD2_ACTIVE_VIDEO_HSTART),
				...
				if (tvafe_dbg_print & TVAFE_DBG_AUTO_HS) {
					tvafe_pr_info("%s:hs_adj_dir:%d,lev:%x,h_dy:%x,0x12d:%x,0x128:%x,0x166:%x\n",
						__func__, cvd2->info.hs_adj_dir, cvd2->info.hs_adj_level,
						h_dynamic_val, R_APB_REG(ACD_REG_2D), R_APB_REG(ACD_REG_28),

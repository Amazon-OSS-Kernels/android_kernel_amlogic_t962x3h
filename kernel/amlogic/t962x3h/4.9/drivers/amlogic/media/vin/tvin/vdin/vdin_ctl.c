...
unsigned int display_ration_bypass;
module_param(display_ration_bypass, uint, 0664);
MODULE_PARM_DESC(display_ration_bypass, "set display_ration_bypass");
...
/*function:whether enable 4k doubewrite to 1280x720
 *T3 vdin doubewrite 720 path:
 *	vdin0:3840x2160-->after linebuffer
 *	      ->scaler(2560x1440)-->shrink(1280x720);
 */
bool vdin_is_en_dw_720(struct vdin_dev_s *devp)
{
	if (devp->vdin_function_sel & VDIN_DW_1280x720 &&
	    devp->index == 0 && vdin_is_4k(devp) && devp->double_wr)
		return true;
	else
		return false;
}

/*function:whether enable 4k doubewrite to 1920x1080
 *T3 vdin doubewrite 1080 path:
 *	vdin0:3840x2160-->after linebuffer->scaler(1920x1080);
 */
bool vdin_is_en_dw_1080(struct vdin_dev_s *devp)
{
	if (devp->vdin_function_sel & VDIN_DW_1920x1080 &&
	    devp->index == 0 && vdin_is_4k(devp) && devp->double_wr)
		return true;
	else
		return false;
		...
		enum bt_path_e bt_path)
{
	...
	unsigned int scale_out_hor = h;

	if (vdin_is_en_dw_720(devp) || vdin_is_en_dw_1080(devp))
		scale_out_hor = devp->prop.scaling4w;
	/* [28:16]         top.input_width_m1   = h-1 */
	/* [12: 0]         top.output_width_m1  = h-1 */
	wr(offset, VDIN_WIDTHM1I_WIDTHM1O, ((h-1)<<16)|(scale_out_hor-1));
	switch ((port)>>8) {
	case 0x01: /* mpeg */
...
vdin_set_color_matrix(enum vdin_matrix_sel_e maxsel,
		       unsigned int offset,
...
void vdin_set_matrix(struct vdin_dev_s *devp)
{
	...
	if (rgb_info_enable == 0) {
		...
		if (!devp->dv.dv_flag) {
			devp->csc_idx = vdin_set_color_matrix(
				maxsel,
				devp->addr_offset,
				devp->fmt_info_p,
				devp->format_convert,
...
static inline void vdin_set_hist_mux(struct vdin_dev_s *devp)
{
	...
	/*
	 * 0: matrix 0 data out
	 * 1: vsc_dout (scaler out)
	 * 2: matrix 1 data out
	 * 3: form matrix1 din
	 */
	wr_bits(devp->addr_offset, VDIN_HIST_CTRL, 1,
 ...
 */
	 ...
	 *	is_meson_m8b_cpu())
	 */
	wr_bits(offset, VDIN_WR_CTRL, 1, VDIN_WRCTRLREG_PAUSE_BIT,
		VDIN_WRCTRLREG_PAUSE_WID);
	if (devp->parm.reserved & PARAM_STATE_WR_MEM_ORDER_STD) {
		...
		/* big endian */
		wr_bits(offset, VDIN_WR_H_START_END, 0, WR_ENDIAN_BIT, WR_ENDIAN_WID);
	}
}
...
void vdin_hw_enable(struct vdin_dev_s *devp)
{
	...
	/* [    4]  top.datapath_en  = 1 */
	wr_bits(offset, VDIN_COM_CTRL0, 1, VDIN_COMMONINPUT_EN_BIT, VDIN_COMMONINPUT_EN_WID);
...
/*
	...
	*/
	...
	if (vdin_is_en_dw_720(devp) || vdin_is_en_dw_1080(devp))
		wr_bits(offset, VDIN_LFIFO_CTRL, 1, PPS_AFTER_LINEBUFF_BIT,
			PPS_AFTER_LINEBUFF_WID);
	else
		devp->h_active = dst_w;
...
static void vdin_set_vscale(struct vdin_dev_s *devp)
{
	...
	if (vdin_is_en_dw_720(devp) || vdin_is_en_dw_1080(devp))
		pr_info("[%s.%d]:v_active:%d, dst_h:%d\n", __func__, devp->index,
			devp->v_active, dst_h);
	else
		devp->v_active = dst_h;
}
...
static void vdin_set_hshrink(struct vdin_dev_s *devp)
{
	...
	if ((hshrk_mode < 2) || (hshrk_mode > 64)) {
		pr_err("vdin.%d hshrink(%d): size is out of range\n",
			devp->index, hshrk_mode);
		return;
	}
...
static void vdin_set_vshrink(struct vdin_dev_s *devp)
{
	...
	unsigned int dst_w = devp->h_shrink_out;
	unsigned int src_h;
	unsigned int dst_h = devp->v_shrink_out;
	...
	if (vdin_is_en_dw_720(devp) || vdin_is_en_dw_1080(devp))
		src_h = devp->prop.scaling4h;
	else
		src_h = devp->v_active;

	vshrk_mode = src_h / dst_h;
	...
	if (cpu_after_eq(MESON_CPU_MAJOR_ID_TM2)) {
		...
		wr_bits(offset, VDIN_VSHRK_SIZE_M1, dst_w - 1,
...
void vdin_set_hvscale(struct vdin_dev_s *devp)
{
	...
	if (vdin_is_en_dw_720(devp)) {
		devp->prop.scaling4w = 2560;
		devp->prop.scaling4h = 1440;
	} else if (vdin_is_en_dw_1080(devp)) {
		devp->prop.scaling4w = 1920;
		devp->prop.scaling4h = 1080;
	}

	pr_info("[vdin.%d] %s hactive:%u,vactive:%u,scaling4w:%u,scaling4h:%u\n",
	...
	if (devp->double_wr && devp->h_active > 1920 && devp->v_active > 1080) {
		...
		if (vdin_is_en_dw_720(devp)) {
			devp->h_shrink_times = H_SHRINK_TIMES_720;
			devp->v_shrink_times = V_SHRINK_TIMES_720;
		} else if (vdin_is_en_dw_1080(devp)) {
	...
	if (vdin_is_en_dw_720(devp) || vdin_is_en_dw_1080(devp)) {
		devp->h_shrink_out = devp->prop.scaling4w / devp->h_shrink_times;
		devp->v_shrink_out = devp->prop.scaling4h / devp->v_shrink_times;
	} else {
	...
	pr_info("[vdin.%d] %s hactive:%u,vactive:%u,hshrkout:%u,vshrkout:%u\n",
		devp->index, __func__, devp->h_active, devp->v_active,
...
enum tvin_color_fmt_e vdin_get_color_fmt(u32 format_convert)
{
	enum tvin_color_fmt_e fmt = TVIN_COLOR_FMT_MAX;
...
enum vdin_buf_fmt_e vdin_get_buf_fmt(enum tvin_color_fmt_e clr_fmt)
{
	enum vdin_buf_fmt_e fmt = BUF_FMT_MAX;
...
void vdin_dolby_pr_meta_data(void *addr, unsigned int size, unsigned int index)
{
	unsigned int i;
	char *c = addr;

	for (i = 0; i < size; i += 16) {
		pr_info("index:%d\t%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
...
static void vdin_dolby_pr_event_meta_data(void *addr, unsigned int size,
			unsigned int index)
{
	unsigned int i;
	char *c = addr;
	...
	for (i = 0; i < size; i += 16) {
		pr_info("meta:%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x:\n",
...
{
	if (dv_dbg_log & BIT(6))
		pr_info("vsif index:%d,size:0x%x pk:0x%x,ver:0x%x,len:0x%x PB:0X%02x %02x %02x %02x\n",
...
void vdin_pr_emp_data(struct vdin_dev_s *devp, struct vframe_s *vf)
{
	if (vdin_isr_monitor & BIT(8))
		pr_info("emp index:%d size:%d id:0x%x data:0x%02x 0x%02x 0x%02x 0x%02x\n",
...
void vdin_dolby_buffer_update(struct vdin_dev_s *devp, unsigned int index)
{
	...
	if (((cnt % dv_dbg_log_du) == 0) && (dv_dbg_log & BIT(5)))
		vdin_dolby_pr_meta_data(c, 128 * rev_cnt, index);
	...
	if ((crc != crc_r) ||
		...
		if (((cnt % dv_dbg_log_du) == 0) && (dv_dbg_log & BIT(3))) {
			...
			pr_info("%s:index:%d dma:%x vaddr:%p size:%d\n",
				...
				devp->vfp->dv_buf_vmem[index],
				meta_size);
			vdin_dolby_pr_meta_data(p, 128, index);
		}
		devp->dv.dv_crc_check = false;
	} else {
		devp->dv.dv_crc_check = true;
		...
		if (((cnt % dv_dbg_log_du) == 0) && (dv_dbg_log & (1 << 4)))
			vdin_dolby_pr_meta_data(c, meta_size, index);
	}
...
int vdin_event_cb(int type, void *data, void *op_arg)
{
	...
	if (type & VFRAME_EVENT_RECEIVER_GET_AUX_DATA) {
		...
		if (dv_dbg_log & BIT(8)) {
			...
			vdin_dolby_pr_event_meta_data(req->aux_buf,
						req->aux_size, index);
		}
		...
		struct vframe_s *vf)
{
	...
	if (IS_HDMI_SRC(devp->parm.port)) {
		...
		if (aspect_ratio == TVIN_ASPECT_4x3_FULL) {
			...
			vf->ratio_control = 0xc0 << DISP_RATIO_ASPECT_RATIO_BIT;
		} else if (aspect_ratio == TVIN_ASPECT_16x9_FULL) {
			vf->pic_mode.screen_mode = VIDEO_WIDEOPTION_CUSTOM;
			vf->pic_mode.provider = PIC_MODE_PROVIDER_WSS;
...
void vdin_dmc_ctrl(struct vdin_dev_s *devp, bool onoff)
{
	...
	if (onoff) {
		...
		if (devp->dtdata->hw_ver == VDIN_HW_T5 ||
			...
			reg = READ_DMCREG(0x60) & (~(1 << 18));
			WRITE_DMCREG(0x60, reg | (1 << 17));
		}
	}
}

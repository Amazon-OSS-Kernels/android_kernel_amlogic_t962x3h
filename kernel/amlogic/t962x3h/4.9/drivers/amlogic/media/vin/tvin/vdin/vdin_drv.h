...
/* 20220406: dv game mode picture display abnormal */
...
/* #define VDIN_DEBUG */
#define VDIN_DW_1280x720		BIT(1)
#define VDIN_DW_1920x1080		BIT(2)
...
#define H_SHRINK_TIMES_720	2
#define V_SHRINK_TIMES_720	2
...
struct vdin_para_to_enc_s {
	...
	/* buffer y data line stride, for alignment referrer to pitch in AMVEncFrameIO */
	unsigned int width_stride;
	/* size limit of the buffer it shall large enough to hold the whole frame */
...
struct vdin_dev_s {
	...
	unsigned int h_shrink_out;/* double write horizon use */
	unsigned int v_shrink_out;/* double write vertical use */

...
#define AO_RTI_STATUS_REG0 0xff800000
...
static void __iomem *bl_status_reg;
static void bl_status_reg_write(int bl_on)
{
	void __iomem *reg = NULL;
	u32 val = 0;

	if (bl_status_reg == NULL) {
		reg = (void __iomem *) ioremap(AO_RTI_STATUS_REG0, 0x4);
		if (reg != NULL)
			bl_status_reg = reg;
	}

	if (bl_status_reg != NULL) {
		val = readl(bl_status_reg);
		if (bl_on == 1)
			writel(val | BIT(28), bl_status_reg);
		else
			writel(val & (~BIT(28)), bl_status_reg);
	}
...
static void bl_power_off(void)
{
	...
	bl_drv->state &= ~BL_STATE_BL_ON;
	backlight_status = 0;
	bl_status_reg_write(0);
	BLPR("backlight power off\n");

#if defined(CONFIG_AMLOGIC_LOCAL_DIMMING) || defined(CONFIG_AMLOGIC_BL_EXTERN)
exit_power_off_bl:
#endif
	mutex_unlock(&bl_power_mutex);
}
...
val = (level - min) * (bl_drv->brightness_level - min) / (max - min) + min;
...
static int aml_bl_config_load_from_unifykey(struct bl_config_s *bconf)
{
	...
	case BL_CTRL_PWM:
		...
		if (bl_pwm->pwm_port == BL_PWM_VS) {
			if ((bl_pwm->pwm_freq > 4) || (bl_pwm->pwm_freq == 0)) {
				BLERR("bl_pwm_vs wrong freq %d\n",
					bl_pwm->pwm_freq);
				bl_pwm->pwm_freq = BL_FREQ_VS_DEFAULT;
			}
...
unsigned int toggle_backlight(unsigned int value)
{
	...
	if (value == OTA_TOGGLE_BACKLIGHT) {
		// AMAZON hope to the backlight can power on
		// when echo the /porc/backlight_status node.
		// Clear bl_off_policy and power on back light
		bl_off_policy = BL_OFF_POLICY_NONE;
		aml_bl_off_policy_cnt = 0;
...
bl_brightness_level_mapping(bl_drv->bldev->props.brightness);
	...
	// AMAZON hope to close the backlight, when use the "recovery,quiescent"
	// command But the aml_bl_prode can power on the backlight.
	// SO:
	// check uboot backlight status: if backlight_status is off, return.
	if (!backlight_status) {
		BLERR("the uboot backlight status is off\n");
		return;
	}
...
// read command line bl_status in uboot.
static int __init aml_bl_status_setup(char *str)
{
	int ret = 0;
...
__setup("bl_status=", aml_bl_status_setup);

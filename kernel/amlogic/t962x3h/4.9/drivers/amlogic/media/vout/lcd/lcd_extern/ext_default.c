				      ...
				      unsigned char data_len)
{
	...
	if (check_offset > data_len)
		check_offset = 0;
	if ((check_offset + check_len) > data_len) {
		EXTERR("check len error(%d)\n", ret);
		goto parameter_err1;
	}
		...
		while ((i + 1) < max_len) {
			...
			if (type == LCD_EXT_CMD_TYPE_NONE) {
				...
				if (size == 1) {
				   ...
				   (type == LCD_EXT_CMD_TYPE_CMD_BIN_DATA2) ||
				...
				ret = lcd_extern_i2c_write(i2c0_dev->client,
				   ...
				   (type == LCD_EXT_CMD_TYPE_CMD2_BIN_DATA2) ||
				...
				ret = lcd_extern_i2c_write(i2c1_dev->client,
				   ...
				   (type == LCD_EXT_CMD_TYPE_CMD3_BIN_DATA2) ||
				...
				ret = lcd_extern_i2c_write(i2c2_dev->client,
				   ...
				   (type == LCD_EXT_CMD_TYPE_CMD4_BIN_DATA2) ||

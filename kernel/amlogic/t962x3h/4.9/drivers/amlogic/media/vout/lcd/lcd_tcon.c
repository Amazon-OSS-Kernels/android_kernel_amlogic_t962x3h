...
int check_model_name_is_UHD_43D6140_T(void)
{
...
int lcd_tcon_is_od_on_keeping_state(void)
{
	return (lcd_tcon_od_sta != 0);
}
...
int lcd_tcon_od_demo_set(int flag)
{
	...
	reg = lcd_tcon_conf->reg_od_demo;
	bit = lcd_tcon_conf->bit_od_demo_en;
	temp = flag ? 1 : 0;
	if (lcd_tcon_conf->core_reg_width == 8)
		lcd_tcon_setb_byte(reg, temp, bit, 1);
	else
...
/*
 * data[0]: lcd_driver id, data[1]: open(1) / close(0) od function
 * data[0] is not used, and remain it for compatible with kernel-5.4
 * because it provided externally
 */
static int lcd_tcon_od_on_off_notifier(struct notifier_block *nb, unsigned long event, void *data)
{
	struct aml_lcd_drv_s *pdrv;
	int val;

	if ((event & LCD_EVENT_TCON_OD_ON_OFF) == 0)
		return NOTIFY_DONE;
	if (!data) {
	...
	if (lcd_tcon_is_od_on_keeping_state()) {
		LCDPR_ISR("%s: od is busy, please try again later\n", __func__);
		return NOTIFY_DONE;
	}
...
static struct notifier_block lcd_tcon_od_on_off_nb = {
	.notifier_call = lcd_tcon_od_on_off_notifier,
};
...
{
	lcd_tcon_od_on_off_state_machine_update();
}
...
static int lcd_tcon_vac_load(void)
{
	...
	if (data_cnt > tcon_rmem.vac_rmem.mem_size)
		data_cnt = tcon_rmem.vac_rmem.mem_size;
...
static int lcd_tcon_demura_set_load(void)
{
	...
	if (data_cnt > tcon_rmem.demura_set_rmem.mem_size)
		data_cnt = tcon_rmem.demura_set_rmem.mem_size;
	data_checksum = buff[4];
	data_lrc = buff[5];
	temp_checksum = lcd_tcon_checksum(&buff[8], data_cnt);
	temp_lrc = lcd_tcon_lrc(&buff[8], data_cnt);
...
static int lcd_tcon_demura_lut_load(void)
{
	...
	if (data_cnt > tcon_rmem.demura_lut_rmem.mem_size)
		data_cnt = tcon_rmem.demura_lut_rmem.mem_size;
	data_checksum = buff[4];
	data_lrc = buff[5];
	temp_checksum = lcd_tcon_checksum(&buff[8], data_cnt);
	temp_lrc = lcd_tcon_lrc(&buff[8], data_cnt);
...
static int lcd_tcon_acc_lut_load(void)
{
	...
	if (data_cnt > tcon_rmem.acc_lut_rmem.mem_size)
		data_cnt = tcon_rmem.acc_lut_rmem.mem_size;
	data_checksum = buff[4];
	data_lrc = buff[5];
	temp_checksum = lcd_tcon_checksum(&buff[8], data_cnt);
	temp_lrc = lcd_tcon_lrc(&buff[8], data_cnt);
...
static int lcd_tcon_get_config(struct aml_lcd_drv_s *lcd_drv)
{
	...
	check_model_name_is_UHD_43D6140_T();
...
static struct lcd_tcon_config_s tcon_data_t5 = {
	...
	.bit_od_demo_en = BIT_OD_DEMO_EN_T5,
...
static struct lcd_tcon_config_s tcon_data_t5d = {
	...
	.bit_od_demo_en = BIT_OD_DEMO_EN_T5D,
...
int lcd_tcon_probe(struct aml_lcd_drv_s *lcd_drv)
{
	...
	ret = aml_lcd_atomic_notifier_register(&lcd_tcon_od_on_off_nb);
	if (ret)
		LCDERR("register lcd_tcon_od_on_off_nb failed\n");
...
int lcd_tcon_remove(struct aml_lcd_drv_s *lcd_drv)
{
	...
	aml_lcd_atomic_notifier_unregister(&lcd_tcon_od_on_off_nb);

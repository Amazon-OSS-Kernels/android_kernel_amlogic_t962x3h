 ...
 * functions
 */
static struct fun_symbol common_func[COMMON_CALLER_SIZE] __initdata = {
	...
	{"codec_mm_alloc_in",		1, 0},
	{"codec_mm_alloc",		1, 0},
	{"codec_mm_alloc_for_dma",	1, 0},
	{"codec_mm_alloc_for_dma_ex",	1, 0},
...
static void print_page_trace(struct zone *zone,
			    struct page_summary *sum, int *mt_cnt)
{
	int i, j;
	struct page_summary *p;
...
void pagetrace_print(void)
{
...
#ifndef CONFIG_64BIT
	if (!trace_buffer) {
		pr_err("page trace not enabled\n");
		return;
	}
#endif
	sum = kzalloc(sizeof(*sum), GFP_ATOMIC | __GFP_NOWARN);
	if (!sum)
		return;

	sum->sum = kzalloc(size, GFP_ATOMIC | __GFP_NOWARN);
	if (!sum->sum) {
		pr_err("alloc failed for pagetrace print\n");
		kfree(sum);
		return;
	}
	...
	for_each_populated_zone(zone) {
		memset(sum->sum, 0, size);
		ret = update_page_trace(NULL, zone, sum->sum, sum->mt_cnt);
		if (ret) {
			pr_err("Error %d in zone %8s\n",
				   ret, zone->name);
			continue;
		}
		print_page_trace(zone, sum->sum, sum->mt_cnt);
	}
	sum->ticks = sched_clock() - sum->ticks;

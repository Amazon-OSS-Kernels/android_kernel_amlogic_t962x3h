		...
		if ((clk_ios >= 200000000) && conf->ddr) {
			...
			if (ret)
				pr_warn("set comp0 as mux_clk parent error\n");
					...
					(host->data->chip_type == MMC_CHIP_T5)) &&
				(clk_ios >= 166000000) &&
			...
			if ((host->data->chip_type == MMC_CHIP_TL1)
				...
				host->gp0_enable = 1;
			} else if ((host->data->chip_type == MMC_CHIP_T5) &&
				(clk_ios <= 195000000)) {
				ret = clk_set_rate(src0_clk, 780000000);
				if (ret)
					pr_warn("not set t5-gp0\n");
				host->gp0_enable = 1;
			}
...
irqreturn_t meson_mmc_irq_thread_v3(int irq, void *dev_id)
{
	...
	case HOST_DAT_CRC_ERR:
		...
		if (aml_card_type_mmc(pdata) &&
			...
			if (host->switch_hs == 1) {
				gintf3->sd_intf3 = !gintf3->sd_intf3;
...
static unsigned long _swap_fixed_adj_win(unsigned long *map,
		u32 shift, u32 div)
{
	DECLARE_BITMAP(left, AML_FIXED_ADJ_MAX);
	DECLARE_BITMAP(right, AML_FIXED_ADJ_MAX);
		...
		struct amlsd_platform *pdata, bool no_cmd)
{
	...
	if (aml_card_type_mmc(pdata)) {
		...
		if (no_cmd) {
			writel(AML_MV_DLY2_NOCMD(step), host->base + SD_EMMC_DELAY2_V3);
			pdata->dly2 = AML_MV_DLY2_NOCMD(step);
		} else {
			writel(AML_MV_DLY2(step), host->base + SD_EMMC_DELAY2_V3);
		...
		if (!no_cmd) {
			writel(AML_MV_DLY2_NOMMC_CMD(step), host->base + SD_EMMC_DELAY2_V3);
		...
		unsigned long *fixed_adj_map, u32 div)
{
	...
	bitmap_copy(prev_map, fixed_adj_map, NBITS_FIX_ADJ);
		...
		*cur_map = _test_fixed_adj(pdata, tuning_data, opcode, 0);
		...
		/* pre adj=core phase-1="hole"&&200MHZ,all line delay+step*/
				if (((ret - 1) == (cop - 1)) && (div == 5))
					...
					set_fixed_adj_line_delay(0, pdata, false);
				return ret;
			}
		...
		if (fir_adj == 0xff)
			continue;
		bitmap_copy(prev_map, cur_map, NBITS_FIX_ADJ);
...
static int find_best_win(char *buf, int num, int *b_s, int *b_sz, bool wrap_f)
{
	int i = 0;
...
static int intf3_scan(struct mmc_host *mmc, u32 opcode, struct aml_tuning_data *tuning_data)
{
	struct amlsd_platform *pdata = mmc_priv(mmc);
	struct amlsd_host *host = pdata->host;
	u32 intf3;
	...
	host->cmd_retune = 1;
	host->is_tunning = 0;
	find_best_win(rx_r, 64, &best_s1, &best_sz1, false);
	find_best_win(rx_f, 64, &best_s2, &best_sz2, false);
...
static void aml_set_tuning_para(struct mmc_host *mmc)
{
	...
	if (pdata->compute_cmd_delay == 1) {
		temp_index = pdata->first_temp_index;
		delay1 = pdata->para.hs4[temp_index].delay1;
		delay2 = pdata->para.hs4[temp_index].delay2;
		intf3 = pdata->para.hs4[temp_index].intf3;

		cmd_delay = ((10 + pdata->compute_coef) * (pdata->cur_temp_index
				- pdata->first_temp_index)) / 10;

		delay2 -= cmd_delay << 24;
		pr_info("record temperature is %d, current temperature is %d, cmd delay is swith from 0x%x to 0x%x\n",
				pdata->first_temp_index,
				pdata->cur_temp_index,
				pdata->para.hs4[temp_index].delay2,
				delay2);
	...
	if (pdata->compute_cmd_delay == 1) {
		for (i = 0; i < 7; i++) {
			if (para->hs4[i].flag == TUNED_FLAG) {
				pdata->first_temp_index = i;
				pdata->cur_temp_index = temp_index;
				para->update = 0;
				return 1;
			}
		}
...
static int scan_mmc_rx_adj_win(struct mmc_host *mmc,
		u32 opcode, struct aml_tuning_data *tuning_data, bool no_cmd)
{
	struct amlsd_platform *pdata = mmc_priv(mmc);
	struct amlsd_host *host = pdata->host;
	unsigned long cur_map[1] = {0};

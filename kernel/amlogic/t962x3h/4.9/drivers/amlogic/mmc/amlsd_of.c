		...
		struct mmc_host *mmc, u32 index)
{
	...
	if (of_node) {
		...
		SD_PARSE_U32_PROP_DEC(child, "compute_cmd_delay",
				      prop, pdata->compute_cmd_delay);
		SD_PARSE_U32_PROP_DEC(child, "compute_coef",
				      prop, pdata->compute_coef);
		SD_PARSE_U32_PROP_DEC(child, "runtime",
				      prop, pdata->runtime);

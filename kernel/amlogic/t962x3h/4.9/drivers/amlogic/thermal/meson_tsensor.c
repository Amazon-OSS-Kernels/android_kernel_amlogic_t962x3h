...
static int __init meson_tsensor_init(void)
{
	return platform_driver_register(&meson_tsensor_driver);
}

static void __init meson_tsensor_exit(void)
{
	return platform_driver_unregister(&meson_tsensor_driver);
}

fs_initcall_sync(meson_tsensor_init);
module_exit(meson_tsensor_exit);

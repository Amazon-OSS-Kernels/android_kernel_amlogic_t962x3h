	...
	struct device_node *node, int id)
{
	...
	if (!strcmp(temp_item->name, "hdcp14_rx")) {
		if (!amlkey_issecure((uint8_t *)temp_item->name) && amlkey_isexsit((uint8_t *)temp_item->name)) {
			//pr_err("%s:%d, amlkey_issecure(temp_item->name) = %d\n", __func__, __LINE__, amlkey_issecure((uint8_t *)temp_item->name));
			//pr_err("%s:%d, amlkey_isexsit(temp_item->name) = %d\n", __func__, __LINE__, amlkey_isexsit((uint8_t *)temp_item->name));
			pr_info("-----%s can not be Secure !\n", temp_item->name);
			temp_item->dev = KEY_M_NORMAL;
		}
	}
	temp_item->permit = 0;

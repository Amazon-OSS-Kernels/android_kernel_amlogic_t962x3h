...
unsigned int eth_phy_setting_cts;
void __iomem *eth_phy_config_cts;
...
static void __iomem *g12a_network_interface_setup(struct platform_device *pdev)
{
	...
	if (!res) {
		...
		tx_amp_src = devm_ioremap(dev, res->start, resource_size(res));
		if (IS_ERR(addr)) {
			dev_err(&pdev->dev,
				"cat't map tx_amp (%d)\n", __LINE__);
			return NULL;
		}
		/*only 0~5 is valid for eth*/
	...
	if (internal_phy) {
		...
		if ((is_meson_t5_cpu()) || (is_meson_t5d_cpu())) {
			if (enet_type == ETH_PHY_T5) {
				...
				writel(cts_setting[cts_amp], ETH_PHY_config_addr + ETH_PLL_CTL3);
				eth_phy_setting_cts = cts_setting[cts_amp];
				eth_phy_config_cts = ETH_PHY_config_addr + ETH_PLL_CTL3;
				tx_amp_bl2 = 0x15;
			} else {
...
//	if ((ee_reset_base) && (support_mac_wol == 0))
	...
	if (is_internal_phy) {
		...
		if ((is_meson_t5_cpu()) || (is_meson_t5d_cpu()))
			if (enet_type == ETH_PHY_T5)
				writel(eth_phy_setting_cts, eth_phy_config_cts);
	}

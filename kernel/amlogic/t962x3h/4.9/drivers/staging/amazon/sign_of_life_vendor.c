...
/*backlight driver will use it to save bl status*/
#define VENDOR_SPECIAL_MODE_BACKLIGHT_STATUS		  BIT(28)
#define VENDOR_UNUSED_MASK	0xf0000000
...
{
	vendor_reg_write(vendor_reg_read() & 0x10000000);
...
int life_cycle_platform_init(sign_of_life_ops *sol_ops)
{
	...
	vendor_breason = vendor_reg_read();
	vendor_breason = vendor_breason & (~VENDOR_UNUSED_MASK);

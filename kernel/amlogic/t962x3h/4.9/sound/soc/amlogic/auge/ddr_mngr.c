				...
				struct toddr_attach *p_attach_resample)
{
	...
	if (p_attach_resample->resample_version >= SM1_RESAMPLE) {
		...
		if (p_attach_resample->resample_version == SM1_RESAMPLE) {
			...
			src_str = toddr_src2str(
				p_attach_resample->attach_module);
			conf = to->chipinfo->to_srcs;
			for (; conf->name[0]; conf++) {
				if (strncmp(conf->name, src_str,
					strlen(src_str)) == 0)
					break;
			}
			new_resample_src_select_v2(p_attach_resample->id,
				conf->val);
		}
	...
	if (p_attach_resample->resample_version >= T5_RESAMPLE &&
		...
		if (to->chipinfo && to->chipinfo->asrc_src_sel_ctrl)
			aml_toddr_set_resample_ab(to,
				p_attach_resample->id, enable);
		else
...
static void frddr_set_sharebuffer_enable(
	struct frddr *fr,  int dst, int lvl, bool enable)
{
 ...
 */
int aml_check_and_release_sharebuffer(struct frddr *fr,
...
unsigned int aml_frddr_get_position(struct frddr *fr)
{
	struct aml_audio_controller *actrl = fr->actrl;
	unsigned int reg_base = fr->reg_base;
	unsigned int ptr1, ptr2, fifo_count;
	unsigned int reg_status1, reg_status2;

	reg_status1 = calc_frddr_address(EE_AUDIO_FRDDR_A_STATUS1, reg_base);
	reg_status2 = calc_frddr_address(EE_AUDIO_FRDDR_A_STATUS2, reg_base);

	do {
		ptr1 = aml_audiobus_read(actrl, reg_status2);
		fifo_count = ((aml_audiobus_read(actrl, reg_status1) >> 8)
				& 0x3ff) << 3;
		ptr2 = aml_audiobus_read(actrl, reg_status2);
	} while (ptr1 != ptr2);

	ptr1 -= fifo_count;
	if (ptr1 < fr->start_addr)
		ptr1 += fr->end_addr - fr->start_addr + FIFO_BURST;

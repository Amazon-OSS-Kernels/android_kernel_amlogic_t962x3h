...
struct earc {
	...
	struct work_struct tx_change_ca_work;
	/*
	 * for ui switch from arc to earc,
	 * it shold be no dmac signal at the beginning earc discovery,
	 * otherwise, the sony soundbar has no sound sometimes.
	 */
	struct work_struct tx_falling_edge_and_mute_work;
	bool tx_falling_edge_flag;
};
...
{
	if (s_earc)
		return get_earctx_enable(s_earc->tx_cmdc_map,
			s_earc->tx_dmac_map);
	return false;
}
...
static irqreturn_t earc_rx_isr(int irq, void *data)
{
	...
	if (p_earc->rx_dmac_clk_on) {
		if (p_earc->rx_status1 &
		    INT_EARCRX_ANA_RST_C_EARCRX_DIV2_HOLD_SET)
			dev_info(p_earc->dev,
				"EARCRX_ANA_RST_C_EARCRX_DIV2_HOLD_SET\n");

		if (p_earc->rx_status1 & INT_EARCRX_ANA_RST_C_NEW_FORMAT_SET) {
			dev_info(p_earc->dev,
				"EARCRX_ANA_RST_C_NEW_FORMAT_SET\n");
		...
		if (p_earc->rx_status1 &
		    INT_EARCRX_ERR_CORRECT_C_BCHERR_INT_SET)
		...
		if (p_earc->rx_status1 &
		    INT_ARCRX_BIPHASE_DECODE_C_VALID_CHANGE)
			dev_dbg(p_earc->dev, "ARCRX_C_VALID_CHANGE\n");
		if (p_earc->rx_status1 &
		    INT_ARCRX_BIPHASE_DECODE_C_FIND_NONPCM2PCM)
			dev_dbg(p_earc->dev, "ARCRX_C_FIND_NONPCM2PCM\n");
		if (p_earc->rx_status1 &
		    INT_ARCRX_BIPHASE_DECODE_C_PCPD_CHANGE)
			dev_dbg(p_earc->dev, "ARCRX_C_PCPD_CHANGE\n");
		if (p_earc->rx_status1 &
		    INT_ARCRX_BIPHASE_DECODE_C_CH_STATUS_CHANGE) {
		...
		if (p_earc->rx_status1 &
		    INT_ARCRX_BIPHASE_DECODE_I_SAMPLE_MODE_CHANGE)
			dev_dbg(p_earc->dev, "ARCRX_I_SAMPLE_MODE_CHANGE\n");
		if (p_earc->chipinfo->rx_dmac_sync_int &&
		    p_earc->rx_status1 &
		    INT_EARCRX_DMAC_VALID_AUTO_NEG_INT_SET) {
			earcrx_dmac_sync_int_enable(p_earc->rx_top_map, 0);
				       ...
				       bool is_earc, bool state)
{
	if (state) {
		if (is_earc) {
			...
			p_earc->first_earctx = true;
		} else {
			/* if the connected device is earc,
			 * lost HB still is earc device
			 */
			if (p_earc->earctx_connected_device_type !=
			    ATNDTYP_EARC)
				p_earc->earctx_connected_device_type =
					ATNDTYP_ARC;
...
static irqreturn_t earc_tx_isr(int irq, void *data)
{
	...
	if (p_earc->tx_dmac_clk_on) {
		...
		if (status1 & INT_EARCTX_ERRCORR_C_FIFO_THD_LESS_PASS)
			dev_info(p_earc->dev, "EARCTX_ECCFIFO_OVERFLOW\n");
		if (status1 & INT_EARCTX_ERRCORR_C_FIFO_OVERFLOW)
			dev_info(p_earc->dev, "EARCTX_ECC_FIFO_OVERFLOW\n");
			      ...
			      unsigned int rate)
{
	...
	dev_dbg(p_earc->dev, "rate %d, set %dX normal dmac clk, p_earc->tx_dmac_freq:%d\n",
...
int aml_earctx_set_audio_coding_type(enum audio_coding_types new_coding_type)
{
	...
	dev_dbg(s_earc->dev, "tx audio coding type: 0x%02x\n", new_coding_type);
	...
	struct frddr *fr, enum aud_codec_types type, int lane_i2s)
{
	...
	if (earc_type == ATNDTYP_DISCNCT) {
		dev_dbg(s_earc->dev,
			"%s: Neither eARC_TX or ARC_TX is attended!\n",
			__func__);
		return -ENOTCONN;
	}
...
void aml_earctx_enable(bool enable)
{
	...
	earctx_enable(s_earc->tx_top_map,
		...
		/* if has falling task, no need open dmac */
		enable && !s_earc->tx_falling_edge_flag);
}
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			...
			earctx_enable(p_earc->tx_top_map,
				      ...
				      p_earc->chipinfo->rterm_on,
				      true);
			if (p_earc->first_earctx) {
				schedule_work(&p_earc->tx_hold_bus_work);
				p_earc->first_earctx = false;
			}
		} else {
...
static void earctx_set_earc_mode(struct earc *p_earc, bool earc_mode)
{
	...
	if (p_earc->tx_earc_mode) {
		...
		earctx_cmdc_arc_connect(p_earc->tx_cmdc_map, false);
		aml_earctx_enable_d2a(true);
		if (p_earc->same_src_on) {
			/* disable dmac before reset hdp */
			earctx_falling_edge_enable(p_earc->tx_dmac_map, false);
			p_earc->tx_falling_edge_flag = true;
			schedule_work(&p_earc->tx_falling_edge_and_mute_work);
		}
		rx_earc_hpd_cntl(); /* reset hpd */
	}
#endif
...
/*
 * eARC TXs and eARC RXs shall treat any modifications to the
 * audio Channel Status bits in the eARC audio stream,
 * with exeption of the MUTE bit changes, as an audio format change.
 * An eARC TX indicates an upcoming audio format change to a connected
 * eARC RX before changing the format  by asserting the Channel
 * Status MUTE bit.
 */
static void tx_change_ca_work_func(struct work_struct *p_work)
{
	struct earc *p_earc = container_of(p_work, struct earc,
					   tx_change_ca_work);

	earctx_set_cs_mute(p_earc->tx_dmac_map, 1);
	earctx_set_cs_ca(p_earc->tx_dmac_map, p_earc->tx_cs_lpcm_ca);
	msleep(300);

	earctx_set_cs_mute(p_earc->tx_dmac_map, 0);
}

static void tx_falling_edge_and_mute_work_func(struct work_struct *p_work)
{
	struct earc *p_earc = container_of(p_work, struct earc,
					   tx_falling_edge_and_mute_work);

	/*
	 * send dmac signal when hpd is pulled up after 700ms ~ 800ms,
			    ...
			    struct snd_ctl_elem_value *ucontrol)
{
	...
	schedule_work(&p_earc->tx_change_ca_work);
...
static int earc_platform_probe(struct platform_device *pdev)
{
	...
	if (p_earc->chipinfo->rx_enable) {
		...
		if (!IS_ERR(p_earc->clk_rx_dmac) &&
			...
			if (ret) {
				dev_err(dev,
					"Can't set clk_rx_dmac parent clk\n");
				return ret;
			}
		}
	...
	if (p_earc->chipinfo->tx_enable) {
		...
		p_earc->clk_tx_cmdc_srcpll = devm_clk_get(&pdev->dev,
			"tx_cmdc_srcpll");
		if (IS_ERR(p_earc->clk_tx_cmdc_srcpll))
			dev_info(&pdev->dev,
				"Check whether support eARC TX\n");

		p_earc->clk_tx_dmac_srcpll = devm_clk_get(&pdev->dev,
			"tx_dmac_srcpll");
		...
		if (!IS_ERR(p_earc->clk_tx_cmdc) &&
			...
			if (ret) {
				dev_err(dev,
					"Can't set clk_tx_cmdc parent clk\n");
				return ret;
			}
		}
		if (!IS_ERR(p_earc->clk_tx_dmac) &&
			...
			if (ret) {
				dev_err(dev,
					"Can't set clk_tx_dmac parent clk\n");
				return ret;
			}
		}
	...
	if (!IS_ERR(p_earc->tx_top_map)) {
		...
		INIT_WORK(&p_earc->tx_change_ca_work, tx_change_ca_work_func);
		INIT_WORK(&p_earc->tx_falling_edge_and_mute_work,
			  tx_falling_edge_and_mute_work_func);
	}

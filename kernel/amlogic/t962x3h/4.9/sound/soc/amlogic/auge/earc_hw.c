...
void earctx_falling_edge_enable(struct regmap *dmac_map, bool enable)
{
	pr_info("%s enable: %d\n", __func__, enable);
	mmio_update_bits(dmac_map, EARCTX_FE_CTRL0, 0x1 << 30, enable << 30);
}
...
void earcrx_cmdc_init(struct regmap *top_map, bool en,
	bool rx_dmac_sync_int, bool rterm_on)
{
	if (rterm_on) {
		      ...
		      bool chnum_mult_mode)
{
	...
	if (chnum_mult_mode)
		mmio_update_bits(dmac_map, EARCRX_SPDIFIN_CTRL6,
			0x1 << 27, 0x1 << 27);
	if (unstable_tick_sel) {
		mmio_update_bits(top_map, EARCRX_DMAC_INT_MASK,
			0x1 << 28, 0x1 << 28);
		mmio_write(dmac_map, EARCRX_DMAC_SYNC_CTRL3,
			0x1 << 19 | 0x2 << 16 | 0x64 << 0);
		mmio_write(dmac_map, EARCRX_DMAC_SYNC_CTRL4,
			0x1 << 19 | 0x2 << 16 | 0x32 << 0);
		mmio_write(dmac_map, EARCRX_DMAC_SYNC_CTRL5,
			0x1 << 19 | 0x2 << 16 | 0x32 << 0);
	}
}
...
static __maybe_unused unsigned int get_cs_ca(enum audio_coding_types coding_type)
{
	unsigned int ca = 0;
		   ...
		   bool falling)
{
	...
	earctx_compressed_enable(dmac_map,
				 ...
				 enable);
	if (falling)
		earctx_falling_edge_enable(dmac_map, enable);
	mmio_update_bits(dmac_map, EARCTX_SPDIFOUT_CTRL0,
	                 0x1 << 31,
	                 enable << 31);
}

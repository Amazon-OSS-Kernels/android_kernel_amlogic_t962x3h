ifneq ($(filter primrose, $(TARGET_BOARD_PLATFORM)),)
LOCAL_PATH := $(call my-dir)

ifneq ($(TARGET_KERNEL_BUILT_FROM_SOURCE), false)

OPTEE_MODULES := $(shell pwd)/$(PRODUCT_OUT)/obj/optee_modules/
OPTEE_DRIVERS := $(shell pwd)/$(VENDOR_AML_PATH_VENDOR)/optee/optee_linuxdriver
KERNEL_OUT_DIR := $(shell pwd)/$(PRODUCT_OUT)/obj/KERNEL_OBJ/

ifeq ($(KERNEL_A32_SUPPORT), true)
KERNEL_ARCH := arm
PREFIX_CROSS_COMPILE := $(abspath $(TOP))/prebuilts/gcc/linux-x86/arm/gcc-linaro-6.3.1-2017.02-x86_64_arm-linux-gnueabihf/bin/arm-linux-gnueabihf-
else
KERNEL_ARCH := arm64
PREFIX_CROSS_COMPILE := $(abspath $(TOP))/prebuilts/gcc/linux-x86/aarch64/linaro-aarch64_linux-2014.09/bin/aarch64-linux-gnu-
endif
